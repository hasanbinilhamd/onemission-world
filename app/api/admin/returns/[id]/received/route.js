...
export async function POST(request, { params }) {
  return withDevTiming(request, async () => {
    ...
    try {
      const response = await orderService.recordReturnReceived({ returnRequestId: params.id, updatedBy: authContext.user.email || authContext.user.name });
      await writeAuditLog({ user: authContext.user, module: 'SALES', action: 'RETURN_RECEIVED', description: `Return request ${params.id} was received.`, metadata: { returnRequestId: params.id } });
