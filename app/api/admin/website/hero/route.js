...
export async function GET(request) {
  return withDevTiming(request, async () => {
    ...
    try {
      const response = await websiteContentService.listAdminHeroItems();
      return NextResponse.json(response);
    } catch (error) {
      return buildWebsiteErrorResponse(error, 'WEBSITE_HERO_FETCH_FAILED');
    }
  });
}
...
export async function PUT(request) {
  return withDevTiming(request, async () => {
    ...
    try {
      ...
      const response = await websiteContentService.updateHeroItems({
        items: payload.items,
        user: authContext.user,
      });
      return NextResponse.json(response);
    } catch (error) {
      return buildWebsiteErrorResponse(error, 'WEBSITE_HERO_UPDATE_FAILED');
    }
  });
}
