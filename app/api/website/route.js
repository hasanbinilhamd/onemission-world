...
function buildWebsiteErrorResponse(error, fallbackCode) {
  const normalizedError = error instanceof WebsiteContentError
    ...
    : new WebsiteContentError({
        message: 'Website content could not be loaded.',
        statusCode: 500,
        code: fallbackCode,
      });
...
export async function GET() {
  try {
    const response = await websiteContentService.getPublicWebsiteContent();
    return NextResponse.json(response);
  } catch (error) {
    return buildWebsiteErrorResponse(error, 'WEBSITE_PUBLIC_FETCH_FAILED');
  }
}
