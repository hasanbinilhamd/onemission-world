...
async function handle(request, { params }) {
  ...
  try {
    ...
    if (segs[0] === 'content') {
      if (method === 'GET' && segs.length === 1) {
        const url = new URL(request.url);
        const { month, year, monthKey } = resolveContentScriptMonthKey({
          month: url.searchParams.get('month') || '',
          year: url.searchParams.get('year') || '',
          legacyMonth: url.searchParams.get('monthKey') || url.searchParams.get('calendarMonth') || '',
        });
        const search = String(url.searchParams.get('search') || '').trim();
        const category = String(url.searchParams.get('category') || '').trim();

        const where = {
          calendarDate: { startsWith: monthKey },
        };
        ...
        const items = await prisma.contentPlanner.findMany({
          where,
          select: {
            id: true,
            title: true,
      ...
      if (method === 'GET' && segs.length === 2) {
        const scriptItem = await prisma.contentPlanner.findUnique({
          where: { id: segs[1] },
          select: {
            id: true,
            title: true,
        ...
        if (!scriptItem) {
          return NextResponse.json({ error: 'Content script item not found.' }, { status: 404 });
        }
        return NextResponse.json(scriptItem);
      }

      if (method === 'POST' && segs.length === 1) {
        const body = await readJson(request);
        const validationError = validateContentScriptPayload(body, { requirePdf: true });
        if (validationError) {
          return NextResponse.json({ error: validationError }, { status: 400 });
        }

        const sanitizedPayload = sanitizeContentScriptPayload(body);
        let storedPdfPayload;
        try {
          storedPdfPayload = buildStoredPdfPayload(body.pdfFile || {});
        } catch (error) {
          return NextResponse.json({ error: error.message || 'Failed to process PDF file.' }, { status: 400 });
        }

        const created = await prisma.contentPlanner.create({
          data: {
            id: uuid(),
            ...sanitizedPayload,
            pdfUrl: storedPdfPayload.pdfUrl,
            pdfFilename: storedPdfPayload.pdfFilename,
          },
          select: {
            id: true,
            title: true,
      ...
      if (method === 'PUT' && segs.length === 2) {
        const body = await readJson(request);
        const existingScript = await prisma.contentPlanner.findUnique({
          where: { id: segs[1] },
          select: {
            id: true,
            title: true,
        ...
        const requirePdf = !(existingScript.pdfUrl && existingScript.pdfFilename) && !body.pdfFile;
        const validationError = validateContentScriptPayload(nextPayload, { requirePdf });
        if (validationError) {
          return NextResponse.json({ error: validationError }, { status: 400 });
        }

        let storedPdfPayload = null;
        if (body.pdfFile) {
          try {
            storedPdfPayload = buildStoredPdfPayload(body.pdfFile || {});
          } catch (error) {
        ...
        const sanitizedPayload = sanitizeContentScriptPayload(nextPayload);
        const updated = await prisma.contentPlanner.update({
          where: { id: segs[1] },
          data: {
            ...sanitizedPayload,
            ...(storedPdfPayload ? {
