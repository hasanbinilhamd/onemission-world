...
const COGS_REVERSAL_JOURNAL_SOURCE = 'COGS Reversal';
const COGS_RESTORE_JOURNAL_SOURCE = 'COGS Restore';
...
export class FinancePostingService {
  ...
  async generateJournalNumber(prismaClient, journalDate) {
    ...
    const latest = await prismaClient.journalEntry.findFirst({
      where: { journalNumber: { startsWith: prefix } },
      select: { journalNumber: true },
      orderBy: { journalNumber: 'desc' },
    });

    const parts = String(latest?.journalNumber || '').split('-');
    const maxSeq = Number.parseInt(parts[parts.length - 1] || '0', 10);
    const nextSeq = Number.isNaN(maxSeq) ? 1 : maxSeq + 1;
  ...
  async findCogsJournalBySource(prismaClient, orderId, source) {
    return prismaClient.journalEntry.findFirst({
      where: {
  ...
  async postCogsCancellationReversal(order, prismaClient = this.prisma) {
    if (!order?.id) {
      throw new FinancePostingError({ message: 'Order is required for COGS cancellation reversal.', statusCode: 400, code: 'FINANCE_POSTING_ORDER_REQUIRED' });
    }

    const existingReversal = await this.findCogsJournalBySource(prismaClient, order.id, COGS_REVERSAL_JOURNAL_SOURCE);
    if (existingReversal) {
      return { journal: existingReversal, action: 'FOUND' };
    }

    const originalCogs = await this.findCogsJournalBySource(prismaClient, order.id, COGS_JOURNAL_SOURCE);
    if (!originalCogs || !Array.isArray(originalCogs.lines) || originalCogs.lines.length === 0) {
      return { journal: null, action: 'SKIPPED' };
    }

    const journalDate = formatJournalDate(this.nowFactory());
    const journalNumber = await this.generateJournalNumber(prismaClient, journalDate);
    const description = `COGS cancellation reversal for Order ${order.orderNumber || order.publicOrderNumber || order.id}`;
    const totalDebit = Number(originalCogs.totalCredit || originalCogs.totalDebit || 0);
    const totalCredit = Number(originalCogs.totalDebit || originalCogs.totalCredit || 0);
    ...
    const journal = await prismaClient.journalEntry.create({
      data: {
        ...
        journalDate,
        description,
        referenceNumber: order.orderNumber || originalCogs.referenceNumber || '',
        journalSource: COGS_REVERSAL_JOURNAL_SOURCE,
        sourceId: order.id,
        journalType: SALES_JOURNAL_TYPE,
        status: 'Posted',
        totalDebit,
        ...
        updatedBy: SALES_JOURNAL_CREATED_BY,
        lines: {
          create: originalCogs.lines.map((line) => ({
            id: this.idGenerator(),
  ...
  async postCogsCancellationRestore(order, prismaClient = this.prisma) {
    if (!order?.id) {
      throw new FinancePostingError({ message: 'Order is required for COGS cancellation restore.', statusCode: 400, code: 'FINANCE_POSTING_ORDER_REQUIRED' });
    }

    const existingRestore = await this.findCogsJournalBySource(prismaClient, order.id, COGS_RESTORE_JOURNAL_SOURCE);
    if (existingRestore) {
      return { journal: existingRestore, action: 'FOUND' };
    }

    const reversal = await this.findCogsJournalBySource(prismaClient, order.id, COGS_REVERSAL_JOURNAL_SOURCE);
    if (!reversal) {
      return { journal: null, action: 'SKIPPED' };
    }
    ...
    const journalDate = formatJournalDate(this.nowFactory());
    const journalNumber = await this.generateJournalNumber(prismaClient, journalDate);
    const description = `COGS restored after cancellation rejection for Order ${order.orderNumber || order.publicOrderNumber || order.id}`;
    ...
    const journal = await prismaClient.journalEntry.create({
      data: {
        ...
        description,
        referenceNumber: order.orderNumber || originalCogs.referenceNumber || '',
        journalSource: COGS_RESTORE_JOURNAL_SOURCE,
        sourceId: order.id,
        journalType: SALES_JOURNAL_TYPE,
        status: 'Posted',
        totalDebit: Number(originalCogs.totalDebit || 0),
        totalCredit: Number(originalCogs.totalCredit || 0),
        createdBy: SALES_JOURNAL_CREATED_BY,
        updatedBy: SALES_JOURNAL_CREATED_BY,
        lines: {
          create: originalCogs.lines.map((line) => ({
