...
const INVENTORY_ASSET_ACCOUNT_CODE = '1500';
...
export class FinancePostingService {
  ...
  async resolveInventoryAssetAccount(prismaClient) {
    const directAccount = await prismaClient.chartOfAccount.findFirst({
      where: {
        accountCode: INVENTORY_ASSET_ACCOUNT_CODE,
        isActive: true,
        allowTransaction: true,
      },
    });
    ...
    throw new FinancePostingError({
      message: 'Inventory asset account mapping could not be resolved.',
      code: 'FINANCE_POSTING_INVENTORY_ACCOUNT_NOT_FOUND',
    });
  }
  ...
  async postCogsJournal(order) {
    ...
    try {
      const journal = await this.prisma.$transaction(async (tx) => {
        ...
        const inventoryAssetAccount = await this.resolveInventoryAssetAccount(tx);
