...
import { verificationTokenService } from '@/lib/verification-token/service';
...
function isValidPhoneNumber(phone) {
  return /^\+?[\d\s\-()]{8,15}$/.test(String(phone || '').trim());
}
...
function buildCustomerResponse(customer) {
  return {
    ...
    emailVerified: Boolean(customer.emailVerified || customer.emailVerifiedAt),
...
async function findExistingCustomerByIdentity(prismaClient, email, phone) {
  const existingByEmail = email
    ? await prismaClient.customer.findUnique({ where: { email } })
    : null;
  ...
  if (phone) {
    return prismaClient.customer.findUnique({ where: { phone } });
  }
...
async function ensureUniqueRegisterIdentity(prismaClient, email, phone) {
  const existingCustomer = await findExistingCustomerByIdentity(prismaClient, email, phone);
  ...
  if (existingCustomer.email && existingCustomer.email === email) {
...
export class CustomerAuthService {
  ...
  async register({
    ...
    const existingGuestCustomer = await ensureUniqueRegisterIdentity(this.prisma, normalizedEmail, normalizedPhone);

    const passwordHash = await hashCustomerPassword(normalizedPassword);
    const customerCode = existingGuestCustomer?.customerCode || await generateCustomerCode(this.prisma);
  ...
  async requestRegistrationVerification({
    ...
    if (!isValidPhoneNumber(normalizedPhone)) {
      throw new CustomerAuthError({
        ...
        statusCode: 400,
        code: 'CUSTOMER_AUTH_PHONE_INVALID',
      });
    }
    ...
    await this.verificationTokens.deleteExpiredVerificationTokens({
      type: 'REGISTER',
      identifier: normalizedEmail,
    });

    const existingVerification = await this.verificationTokens.getVerificationToken({
      type: 'REGISTER',
      identifier: normalizedEmail,
    });

    const verification = existingVerification
      ? await this.verificationTokens.resendVerificationToken({
          type: 'REGISTER',
          identifier: normalizedEmail,
          payload: verificationPayload,
        })
      : await this.verificationTokens.createVerificationToken({
          type: 'REGISTER',
          identifier: normalizedEmail,
          payload: verificationPayload,
        });
    ...
    try {
      ...
      logCustomerAuthAudit('REGISTER_OTP_FAILED', {
        email: normalizedEmail,
        reason: error instanceof Error ? error.message : 'Unable to send OTP email.',
      });
      throw new CustomerAuthError({
        message: 'Unable to send the verification code right now. Please try again.',
        statusCode: 503,
        code: 'CUSTOMER_AUTH_REGISTER_OTP_EMAIL_FAILED',
      });
    }

    logCustomerAuthAudit(existingVerification ? 'REGISTER_OTP_RESENT' : 'REGISTER_OTP_SENT', {
      email: normalizedEmail,
      customerName: normalizedCustomerName,
  ...
  async verifyRegistrationOtp({ email, otp, request, device = '' }) {
    const normalizedEmail = normalizeEmail(email);
    ...
    if (!normalizedOtp) {
      throw new CustomerAuthError({
        ...
        statusCode: 400,
        code: 'CUSTOMER_AUTH_REGISTER_OTP_REQUIRED',
      });
    }

    let verificationToken;
    try {
      verificationToken = await this.verificationTokens.verifyOtp({
        type: 'REGISTER',
        identifier: normalizedEmail,
        otp: normalizedOtp,
      });
      ...
      if (error?.code === 'VERIFICATION_TOKEN_EXPIRED') {
        logCustomerAuthAudit('REGISTER_OTP_EXPIRED', {
          email: normalizedEmail,
        });
      } else {
        logCustomerAuthAudit('REGISTER_OTP_FAILED', {
    ...
    const payload = verificationToken.payload && typeof verificationToken.payload === 'object'
      ? verificationToken.payload
      : {};
    const normalizedCustomerName = normalizeCustomerName(payload.customerName);
    const normalizedPhone = normalizePhone(payload.phone);
    const passwordHash = String(payload.passwordHash || '').trim();

    if (!normalizedCustomerName || !normalizedPhone || !passwordHash) {
      throw new CustomerAuthError({
        message: 'Registration verification payload is invalid.',
        statusCode: 400,
        code: 'CUSTOMER_AUTH_REGISTER_PAYLOAD_INVALID',
      });
    }
    ...
    await this.verificationTokens.deleteVerificationToken(verificationToken.id);

    logCustomerAuthAudit('REGISTER_OTP_VERIFIED', {
      email: normalizedEmail,
      customerId: customer.id,
      customerCode: customer.customerCode,
    });
  ...
  async resendRegistrationOtp({ email }) {
    ...
    let verification;
    try {
      verification = await this.verificationTokens.resendVerificationToken({
        type: 'REGISTER',
        identifier: normalizedEmail,
      });
    } catch (error) {
      if (error?.code === 'VERIFICATION_TOKEN_EXPIRED') {
        ...
        logCustomerAuthAudit('REGISTER_OTP_FAILED', {
          email: normalizedEmail,
          reason: error?.message || 'Unable to resend OTP.',
        });
      }
      throw error;
    }

    const payload = verification.verificationToken.payload && typeof verification.verificationToken.payload === 'object'
      ? verification.verificationToken.payload
      : {};
    const normalizedCustomerName = normalizeCustomerName(payload.customerName, normalizedEmail);
    ...
    try {
      ...
      logCustomerAuthAudit('REGISTER_OTP_FAILED', {
        email: normalizedEmail,
        reason: error instanceof Error ? error.message : 'Unable to resend OTP email.',
      });
      throw new CustomerAuthError({
        message: 'Unable to send the verification code right now. Please try again.',
        statusCode: 503,
    ...
    logCustomerAuthAudit('REGISTER_OTP_RESENT', {
      email: normalizedEmail,
      resendCount: verification.verificationToken.resendCount,
      expiresAt: verification.expiresAt,
    });
