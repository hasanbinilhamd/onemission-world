...
async function generateCustomerCode(prismaClient) {
...
async function createCustomerSession(prismaClient, { customer, request, device = '' }) {
...
export class CustomerAuthService {
  ...
  async loginWithGoogle({ idToken, request, device = '' }) {
    ...
    if (!customer) {
      ...
      customer = await this.prisma.customer.create({
        data: {
          id: uuid(),
          customerCode,
          customerName: googleProfile.customerName || googleProfile.email,
          email: googleProfile.email,
          googleId: googleProfile.googleId,
      ...
      customer = await this.prisma.customer.update({
        where: { id: customer.id },
        data: {
          customerName: customer.customerName || googleProfile.customerName || googleProfile.email,
