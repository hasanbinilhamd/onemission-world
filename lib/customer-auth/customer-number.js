...
function generateCustomerNumberCandidate(prefix = CUSTOMER_NUMBER_PREFIX) {
...
export async function generateCustomerCode(prismaClient) {
  const systemSettings = await getSystemSettingsMap(prismaClient);
  const configuredPrefix = String(systemSettings.customer_prefix?.value || CUSTOMER_NUMBER_PREFIX).trim().toUpperCase() || CUSTOMER_NUMBER_PREFIX;

  for (let attempt = 1; attempt <= CUSTOMER_NUMBER_RETRY_LIMIT; attempt += 1) {
    const candidate = generateCustomerNumberCandidate(configuredPrefix);
