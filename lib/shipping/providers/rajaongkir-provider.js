...
function normalizeDistrictCollection(data) {
  ...
  if (data.data && typeof data.data === 'object') {
    return normalizeDistrictCollection(data.data);
  }
  ...
  if (data.results && typeof data.results === 'object') {
    return normalizeDistrictCollection(data.results);
  }
  ...
  return values.flatMap((item) => normalizeDistrictCollection(item));
}
...
export class RajaOngkirProvider {
  ...
  async getDistricts(cityId) {
    ...
    try {
      ...
      console.error('[Shipping:RajaOngkirProvider:getDistricts:error]', {
        requestedUrl,
        providerErrorMessage: error?.message || 'Unknown provider error.',
        providerErrorDetails: error?.details || '',
      });
      throw error;
    }
  }
  ...
  async request({ method, endpoint, resource, headers = {}, body, includeDiagnostics = false }) {
    ...
    try {
      ...
      if (!response.ok) {
        console.error('[Shipping:RajaOngkirProvider:request:error]', {
          requestedUrl,
          httpStatus: response.status,
          rawJsonResponse: rawText,
          parsedResponseBody: parsedJson,
        });
