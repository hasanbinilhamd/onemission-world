...
function toPostalCodeNumber(value) {
  const normalized = normalizePostalCode(value);
  return normalized.length === 5 ? Number(normalized) : null;
}

function normalizeCouriers(courier) {
  const configured = String(process.env.BITESHIP_RATES_COURIERS || 'anteraja,jne,sicepat')
    .split(',')
...
export class BiteshipProvider {
  ...
  async getShippingCost({ originDistrictId, destinationDistrictId, destinationPostalCode, weight, courier }) {
    ...
    const originPostalCodeNumber = toPostalCodeNumber(originPostalCode);
    const destinationPostal = normalizePostalCode(destinationPostalCode);
    const destinationPostalNumber = toPostalCodeNumber(destinationPostal);
    const couriers = normalizeCouriers(courier);

    if (!originPostalCodeNumber && !config.origin.areaId) {
      throw new ShippingModuleError({
        message: 'Origin postal code must be a valid 5-digit postal code for Biteship shipping rates.',
        statusCode: 400,
        code: 'SHIPPING_BITESHIP_ORIGIN_POSTAL_INVALID',
      });
    }
    if (!destinationPostalNumber) {
      throw new ShippingModuleError({
        message: 'Destination postal code must be a valid 5-digit postal code for Biteship shipping rates.',
        statusCode: 400,
        code: 'SHIPPING_BITESHIP_DESTINATION_POSTAL_INVALID',
      });
    }

    const payload = {
      ...(originPostalCodeNumber ? { origin_postal_code: originPostalCodeNumber } : { origin_area_id: config.origin.areaId }),
      destination_postal_code: destinationPostalNumber,
      couriers,
      items: [
        {
    ...
    try {
      console.info('[Shipping:BiteshipProvider:getShippingCost]', {
        originPostalCode: originPostalCodeNumber || null,
        destinationPostalCode: destinationPostalNumber,
        couriers,
      });
      const response = await this.client.retrieveRates(payload);
      const originProviderId = originPostalCodeNumber ? `postal:${originPostalCode}` : config.origin.areaId;
      const destinationProviderId = `postal:${destinationPostal}`;
