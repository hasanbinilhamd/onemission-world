...
export function validateOriginConfig(origin) {
  const missing = [];
  ...
  const hasLatitude = origin.latitude !== null && origin.latitude !== undefined;
  const hasLongitude = origin.longitude !== null && origin.longitude !== undefined;
  if (hasLatitude !== hasLongitude) {
    const missingCoordinateEnv = hasLatitude ? 'BITESHIP_ORIGIN_LONGITUDE' : 'BITESHIP_ORIGIN_LATITUDE';
    throw new OrderError({
      message: `Biteship origin coordinate configuration is incomplete: ${missingCoordinateEnv} is missing while the other coordinate is configured.`,
      statusCode: 400,
      code: 'BITESHIP_ORIGIN_COORDINATE_CONFIG_INCOMPLETE',
    });
  }
  if (hasLatitude) {
    ...
    if (!Number.isFinite(latitude) || latitude < -90 || latitude > 90) {
      throw new OrderError({
        message: 'Biteship origin latitude must be a number between -90 and 90.',
        statusCode: 400,
        code: 'BITESHIP_ORIGIN_COORDINATE_CONFIG_INVALID',
      });
    }
    if (!Number.isFinite(longitude) || longitude < -180 || longitude > 180) {
      throw new OrderError({
        message: 'Biteship origin longitude must be a number between -180 and 180.',
        statusCode: 400,
        code: 'BITESHIP_ORIGIN_COORDINATE_CONFIG_INVALID',
      });
    }
...
export function buildCreateOrderPayload({ order, courierCompany = '', courierType = '', config }) {
  ...
  const payload = {
    ...
    ...(config.origin.latitude !== null && config.origin.latitude !== undefined
      ? {
          origin_coordinate: {
