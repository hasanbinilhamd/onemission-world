...
function drawImageFit(page, image, x, topY, boxWidth, boxHeight) {
  if (!image) return;
...
function drawShippingLabelPage({ page, order, fonts, logoImage, courierLogoImage, format = 'barcode' }) {
  ...
  if (logoImage) drawImageFit(page, logoImage, x, 8, 92, 17);
  else drawText(page, 'ONEMISSION', x, 10, 11, fonts.bold, { width: 96 });
  drawText(page, `SHIPPING LABEL · ${routingCode}`, x, 24, 4.8, fonts.bold, { color: MUTED, width: 110 });
