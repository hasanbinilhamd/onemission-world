...
const COURIER_NAME_ALIASES = [
  { code: 'jne', patterns: ['jne', 'jalur nugraha ekakurir'] },
  { code: 'jnt', patterns: ['j&t', 'jnt', 'jet express'] },
  { code: 'sicepat', patterns: ['sicepat'] },
  { code: 'pos', patterns: ['pos indonesia', 'pos'] },
  { code: 'anteraja', patterns: ['anteraja'] },
  { code: 'ninja', patterns: ['ninja express', 'ninja'] },
];

function normalizeCourierCode(...values) {
  const normalizedValues = values
    .map((value) => toStringValue(value).toLowerCase())
    .filter(Boolean);

  for (const value of normalizedValues) {
    const aliasMatch = COURIER_NAME_ALIASES.find((alias) =>
      alias.patterns.some((pattern) => value.includes(pattern)),
    );
...
export function mapShippingCostResponse(record, fallbackCourier) {
  ...
  const courierCode = normalizeCourierCode(record.code, courierName, fallbackCourier);
...
export function mapProvinceApiResponse(record) {
  return {
...
export function mapCityApiResponse(record) {
  return {
    id: toStringValue(record.id),
    provinceId: toStringValue(record.province_id),
    name: toStringValue(record.name),
    type: toStringValue(record.type),
    postalCode: toStringValue(record.postal_code),
  };
}

export function mapDistrictApiResponse(record, provider = '') {
  return {
    id: toStringValue(record.id),
    cityId: toStringValue(record.city_id),
    name: toStringValue(record.name),
...
export function mapShippingCostApiResponse(record) {
  return {
    courier: toStringValue(record.courier),
    courierName: toStringValue(record.courierName),
    service: toStringValue(record.service),
    description: toStringValue(record.description),
    estimatedDelivery: toStringValue(record.estimated_delivery),
    cost: toNumberValue(record.cost),
    shippingProvider: toStringValue(record.shippingProvider),
  };
}
