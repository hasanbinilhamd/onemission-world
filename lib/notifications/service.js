...
const EVENT_SETTING_KEYS = {
  ...
  [NOTIFICATION_EVENT_TYPE.REFUND_REQUESTED]: 'internal_refund_requested',
  [NOTIFICATION_EVENT_TYPE.REPLACEMENT_REQUESTED]: 'internal_replacement_requested',
...
const DEFAULT_NOTIFICATION_SETTINGS = [
  ...
  { category: 'internal', settingKey: 'internal_refund_requested', label: 'Refund Requested', isEnabled: true },
  { category: 'internal', settingKey: 'internal_replacement_requested', label: 'Replacement Requested', isEnabled: true },
...
function eventDefinition(type, payload = {}) {
  const definitions = {
    ...
    [NOTIFICATION_EVENT_TYPE.REFUND_REQUESTED]: {
      module: 'SALES',
      severity: 'warning',
      title: payload.resolution === 'REPLACEMENT' ? 'Replacement Requested' : 'Refund Requested',
      message: `${payload.resolution === 'REPLACEMENT' ? 'Replacement' : 'Refund'} request was submitted for order ${payload.publicOrderNumber || payload.orderNumber || ''} by ${payload.customerName || 'customer'}.`,
      actionUrl: 'refundrequests',
      actionLabel: 'Open Return Requests',
      referenceId: payload.publicOrderNumber || payload.orderNumber || '',
    },
    [NOTIFICATION_EVENT_TYPE.REPLACEMENT_REQUESTED]: {
      module: 'SALES',
      severity: 'warning',
      title: 'Replacement Requested',
      message: `Replacement request was submitted for order ${payload.publicOrderNumber || payload.orderNumber || ''} by ${payload.customerName || 'customer'}.`,
      actionUrl: 'refundrequests',
      actionLabel: 'Open Return Requests',
      referenceId: payload.publicOrderNumber || payload.orderNumber || '',
    },
