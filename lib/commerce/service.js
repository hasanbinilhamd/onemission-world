...
export class CommerceProductService {
  ...
  async buildProductListResponse({ filters, baseUrl }) {
    ...
    const reviewSummaryMap = await getPublishedProductReviewSummaryMap(this.prisma, products.map((product) => product.id));
    const snapshots = this.buildCommerceSnapshots(products, baseUrl, reviewSummaryMap);
  ...
  async listProducts({ query = {}, baseUrl }) {
    ...
    return getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.COLLECTION, async () => {
  ...
  async getProductBySlug({ slug, baseUrl }) {
    ...
    return getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.DETAIL, async () => {
      const allPublishedProducts = await this.fetchPublishedProducts();
      const reviewSummaryMap = await getPublishedProductReviewSummaryMap(this.prisma, allPublishedProducts.map((entry) => entry.id));
      const snapshots = this.buildCommerceSnapshots(allPublishedProducts, baseUrl, reviewSummaryMap);
  ...
  async listFeaturedProducts({ query = {}, baseUrl }) {
    ...
    return getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.FEATURED, async () => {
      return this.buildProductListResponse({ filters, baseUrl });
    });
  }
  ...
  async searchProducts({ query = {}, baseUrl }) {
    ...
    return getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.SEARCH, async () => {
      return this.buildProductListResponse({ filters, baseUrl });
    });
  }
}
