...
function mapVariant(product, variant, baseUrl) {
  const stock = Number(variant.websiteStock ?? variant.quantity) || 0;
...
function buildCardProduct({ product, baseUrl, newArrival }) {
  ...
  const hoverImage = buildAbsoluteUrl(baseUrl, product.hoverImageUrl || product.imageUrl);
  const variants = product.inventory.filter((entry) => (entry.status || 'Active') === 'Active');
  const totalStock = variants.reduce((sum, entry) => sum + (Number(entry.websiteStock ?? entry.quantity) || 0), 0);
...
export class CommerceProductService {
  ...
  buildCommerceSnapshots(products, baseUrl, reviewSummaryMap = new Map()) {
    ...
    return products.map((product) => {
      ...
      const galleryMedia = Array.isArray(product.galleryItems)
        ? product.galleryItems
            .map((item, index) => ({
              id: item.id,
              mediaUrl: buildAbsoluteUrl(baseUrl, item.mediaUrl),
              mediaType: normalizeGalleryMediaType(item.mediaType),
              sortOrder: Number.isFinite(Number(item.sortOrder)) ? Number(item.sortOrder) : index + 1,
            }))
            .sort((left, right) => left.sortOrder - right.sortOrder || left.id.localeCompare(right.id))
        : [];
      const fallbackGalleryMedia = galleryMedia.length > 0
        ? galleryMedia
        : [{
            id: `${product.id}-thumbnail`,
            mediaUrl: card.thumbnail,
            mediaType: 'image',
            sortOrder: 1,
          }];
      const productShowcaseItems = Array.isArray(product.showcaseItems)
        ? product.showcaseItems
            .filter((item) => item.isActive && String(item.mediaUrl || '').trim())
            .map((item, index) => ({
              id: item.id,
              mediaUrl: buildAbsoluteUrl(baseUrl, item.mediaUrl),
              mediaType: normalizeGalleryMediaType(item.mediaType),
      ...
      return {
        ...
        shippingInformation: String(product.shippingInformation || '').trim(),
        sizeGuideImageUrl: String(product.sizeGuideImageUrl || '').trim()
          ? buildAbsoluteUrl(baseUrl, product.sizeGuideImageUrl)
          : '',
        averageRating: Number(reviewSummary.averageRating || 0),
        rating: Number(reviewSummary.averageRating || 0),
        reviewCount: Number(reviewSummary.reviewCount || 0),
        gallery: fallbackGalleryMedia.filter((item) => item.mediaType === 'image').map((item) => item.mediaUrl),
        galleryMedia: fallbackGalleryMedia,
  ...
  async buildProductListResponse({ filters, baseUrl }) {
  ...
  async attachReviewSummaryToProductCollectionResponse(response) {
    const items = Array.isArray(response?.data) ? response.data : [];
    if (items.length === 0) {
      return response;
    }

    const summaryMap = await getPublishedProductReviewSummaryMap(this.prisma, items.map((item) => item.id));

    return {
      ...response,
      data: items.map((item) => {
        const summary = summaryMap.get(item.id) || { averageRating: 0, reviewCount: 0 };
        return {
          ...item,
  ...
  async attachReviewSummaryToProductDetailResponse(response) {
    const product = response?.data;
    if (!product?.id) {
      return response;
    }

    const summaryMap = await getPublishedProductReviewSummaryMap(this.prisma, [product.id]);
    const summary = summaryMap.get(product.id) || { averageRating: 0, reviewCount: 0 };
  ...
  async listProducts({ query = {}, baseUrl }) {
    ...
    const cacheKey = buildCollectionCacheKey({ baseUrl, filters });

    const response = await getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.COLLECTION, async () => {
      return this.buildProductListResponse({ filters, baseUrl });
    });

    return this.attachReviewSummaryToProductCollectionResponse(response);
  }
  ...
  async getProductBySlug({ slug, baseUrl }) {
    ...
    const cacheKey = buildProductDetailCacheKey({
      baseUrl,
      slug: normalizedSlug,
    });

    const response = await getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.DETAIL, async () => {
      ...
      const product = snapshots.find((entry) => entry.slug === normalizedSlug);
      ...
      return {
        data: {
          ...
          galleryMedia: product.galleryMedia,
          productShowcaseItems: product.productShowcaseItems,
    ...
    return this.attachReviewSummaryToProductDetailResponse(response);
  }

  async listCategories({ baseUrl }) {
    const cacheKey = buildProductCategoriesCacheKey({ baseUrl });

    return getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.CATEGORIES, async () => {
      const products = await this.fetchPublishedProducts();
      const snapshots = this.buildCommerceSnapshots(products, baseUrl);
  ...
  async listFeaturedProducts({ query = {}, baseUrl }) {
    const filters = this.normalizeListQuery({
      ...query,
      featured: 'true',
    });
    const cacheKey = buildFeaturedCacheKey({ baseUrl, filters });

    const response = await getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.FEATURED, async () => {
      return this.buildProductListResponse({ filters, baseUrl });
    });
  ...
  async searchProducts({ query = {}, baseUrl }) {
    ...
    const cacheKey = buildProductSearchCacheKey({
      baseUrl,
      search: searchValue || 'all',
      filters,
    });

    const response = await getProductCacheValue(cacheKey, PRODUCT_CACHE_TTL_SECONDS.SEARCH, async () => {
      return this.buildProductListResponse({ filters, baseUrl });
    });
