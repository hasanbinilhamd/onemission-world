import { v4 as uuid } from 'uuid';
import { prisma } from '@/lib/prisma';
import { shippingService } from '@/lib/shipping';
import { shippingConfig } from '@/lib/shipping/config';
import { generateCustomerCode } from '@/lib/customer-auth/customer-number';
import { getCachedValue } from '@/lib/server-cache';
import { calculateGrandTotal, calculateItemSubtotal, calculateSubtotal } from './calculations';
import { promotionService } from '@/lib/promotions';
import { CheckoutModuleError } from './errors';

export const CHECKOUT_STATUS = {
  DRAFT: 'DRAFT',
  PENDING: 'PENDING',
  PAID: 'PAID',
  EXPIRED: 'EXPIRED',
  CANCELLED: 'CANCELLED',
  COMPLETED: 'COMPLETED',
};

const DEFAULT_CURRENCY = 'IDR';
const DEFAULT_EXPIRATION_HOURS = 24;
const SALES_CHANNEL_CACHE_TTL_MS = 300_000;

function logCheckoutValidation({ checkoutNumber = '', status = '', validationResult, durationMs }) {
  const payload = {
    checkoutNumber,
    status,
    validationResult,
    durationMs,
  };

  if (validationResult === 'FAILED' || validationResult === 'EXPIRED' || validationResult === 'LOCKED') {
    console.warn('[CheckoutService]', payload);
    return;
  }

  console.log('[CheckoutService]', payload);
}

function normalizeWeight(value) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
}

export class CheckoutService {
  constructor({
    prismaClient = prisma,
    shipping = shippingService,
    idGenerator = uuid,
    nowFactory = () => new Date(),
  } = {}) {
    this.prisma = prismaClient;
    this.shippingService = shipping;
    this.idGenerator = idGenerator;
    this.nowFactory = nowFactory;
  }

  getShippingCostExecutor() {
    if (typeof this.shippingService.calculateShippingCost === 'function') {
      return this.shippingService.calculateShippingCost.bind(this.shippingService);
    }

    return this.shippingService.getShippingCost.bind(this.shippingService);
  }

  async generateCheckoutNumber() {
    const now = this.nowFactory();
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, '0');
    const prefix = `CHK-${year}${month}-`;

    const existing = await this.prisma.checkoutSession.findMany({
      where: { checkoutNumber: { startsWith: prefix } },
      select: { checkoutNumber: true },
      orderBy: { checkoutNumber: 'desc' },
    });

    let maxSeq = 0;
    for (const entry of existing) {
      const parts = entry.checkoutNumber.split('-');
      const seq = parseInt(parts[parts.length - 1] || '0', 10);
      if (!Number.isNaN(seq) && seq > maxSeq) {
        maxSeq = seq;
      }
    }

    return `${prefix}${String(maxSeq + 1).padStart(5, '0')}`;
  }

  async validateCustomer(customerId) {
    if (!customerId) {
      throw new CheckoutModuleError({
        message: 'customerId is required.',
        statusCode: 400,
        code: 'CHECKOUT_CUSTOMER_REQUIRED',
      });
    }

    const customer = await this.prisma.customer.findUnique({ where: { id: customerId } });
    if (!customer || customer.status !== 'Active') {
      throw new CheckoutModuleError({
        message: 'Customer was not found.',
        statusCode: 404,
        code: 'CHECKOUT_CUSTOMER_NOT_FOUND',
      });
    }

    return customer;
  }

  async ensureCheckoutCustomer({ customerId, customer }) {
    if (customerId) {
      return this.validateCustomer(customerId);
    }

    const customerName = String(customer?.customerName || customer?.fullName || '').trim();
    const email = String(customer?.email || '').trim().toLowerCase();
    const phone = String(customer?.phone || '').trim();
    const customerType = String(customer?.customerType || 'Individual').trim() || 'Individual';

    if (!customerName) {
      throw new CheckoutModuleError({
        message: 'customer.customerName is required.',
        statusCode: 400,
        code: 'CHECKOUT_CUSTOMER_NAME_REQUIRED',
      });
    }

    if (!email && !phone) {
      throw new CheckoutModuleError({
        message: 'At least one of customer email or phone is required.',
        statusCode: 400,
        code: 'CHECKOUT_CUSTOMER_CONTACT_REQUIRED',
      });
    }

    let existingCustomer = null;
    if (email) {
      existingCustomer = await this.prisma.customer.findUnique({ where: { email } });
    }
    if (!existingCustomer && phone) {
      existingCustomer = await this.prisma.customer.findUnique({ where: { phone } });
    }

    if (existingCustomer) {
      if ((existingCustomer.status || 'Active') !== 'Active') {
        throw new CheckoutModuleError({
          message: 'Customer account is inactive.',
          statusCode: 403,
          code: 'CHECKOUT_CUSTOMER_INACTIVE',
        });
      }

      if (!existingCustomer.isGuest) {
        throw new CheckoutModuleError({
          message: 'An account with this email already exists. Please login first.',
          statusCode: 409,
          code: 'CHECKOUT_CUSTOMER_LOGIN_REQUIRED',
        });
      }

      const updateData = {};
      if ((!existingCustomer.customerName || existingCustomer.isGuest) && customerName) updateData.customerName = customerName;
      if (!existingCustomer.email && email) updateData.email = email;
      if (!existingCustomer.phone && phone) updateData.phone = phone;
      if (Object.keys(updateData).length > 0) {
        existingCustomer = await this.prisma.customer.update({
          where: { id: existingCustomer.id },
          data: updateData,
        });
      }

      return existingCustomer;
    }

    const customerCode = await generateCustomerCode(this.prisma);
    return this.prisma.customer.create({
      data: {
        id: this.idGenerator(),
        customerCode,
        customerName,
        email: email || null,
        phone: phone || null,
        customerType,
        isGuest: true,
        passwordHash: null,
        emailVerified: false,
        status: 'Active',
      },
    });
  }

  async resolveDefaultSalesChannel() {
    const channels = await getCachedValue('master:saleschannels:active-default', SALES_CHANNEL_CACHE_TTL_MS, async () => (
      this.prisma.salesChannel.findMany({
        where: { status: 'Active' },
        orderBy: [
          { isDefault: 'desc' },
          { channelName: 'asc' },
        ],
      })
    ));

    const selected = channels.find((channel) => channel.isDefault)
      || channels.find((channel) => String(channel.channelName || '').trim().toLowerCase() === 'website')
      || channels[0]
      || null;

    if (!selected) {
      throw new CheckoutModuleError({
        message: 'Sales channel was not found.',
        statusCode: 404,
        code: 'CHECKOUT_SALES_CHANNEL_NOT_FOUND',
      });
    }

    return selected;
  }

  async validateSalesChannel(salesChannelId) {
    if (!salesChannelId) {
      return this.resolveDefaultSalesChannel();
    }

    const salesChannel = await this.prisma.salesChannel.findUnique({ where: { id: salesChannelId } });
    if (!salesChannel || salesChannel.status !== 'Active') {
      throw new CheckoutModuleError({
        message: 'Sales channel was not found.',
        statusCode: 404,
        code: 'CHECKOUT_SALES_CHANNEL_NOT_FOUND',
      });
    }

    return salesChannel;
  }

  async validateItems(items, currency) {
    if (!Array.isArray(items) || items.length === 0) {
      throw new CheckoutModuleError({
        message: 'At least one checkout item is required.',
        statusCode: 400,
        code: 'CHECKOUT_ITEMS_REQUIRED',
      });
    }

    const snapshots = [];

    for (const item of items) {
      const productId = item?.productId;
      const variantId = item?.variantId;
      const quantity = Number(item?.qty ?? item?.quantity);

      if (!productId || !variantId) {
        throw new CheckoutModuleError({
          message: 'productId and variantId are required for each item.',
          statusCode: 400,
          code: 'CHECKOUT_ITEM_IDENTIFIERS_REQUIRED',
        });
      }

      if (!Number.isInteger(quantity) || quantity <= 0) {
        throw new CheckoutModuleError({
          message: 'qty must be a positive integer for each item.',
          statusCode: 400,
          code: 'CHECKOUT_INVALID_QUANTITY',
        });
      }

      const product = await this.prisma.product.findUnique({ where: { id: productId } });
      if (!product) {
        throw new CheckoutModuleError({
          message: 'Product was not found.',
          statusCode: 404,
          code: 'CHECKOUT_PRODUCT_NOT_FOUND',
        });
      }

      if (product.status !== 'Active') {
        throw new CheckoutModuleError({
          message: 'Product is inactive.',
          statusCode: 400,
          code: 'CHECKOUT_PRODUCT_INACTIVE',
        });
      }

      const variant = await this.prisma.inventory.findUnique({ where: { id: variantId } });
      if (!variant) {
        throw new CheckoutModuleError({
          message: 'Variant was not found.',
          statusCode: 404,
          code: 'CHECKOUT_VARIANT_NOT_FOUND',
        });
      }

      if (variant.productId !== productId) {
        throw new CheckoutModuleError({
          message: 'Variant does not belong to the selected product.',
          statusCode: 400,
          code: 'CHECKOUT_VARIANT_PRODUCT_MISMATCH',
        });
      }

      if ((variant.status || 'Active') !== 'Active') {
        throw new CheckoutModuleError({
          message: 'Variant is inactive.',
          statusCode: 400,
          code: 'CHECKOUT_VARIANT_INACTIVE',
        });
      }

      if (Number(variant.websiteStock ?? variant.quantity ?? 0) < quantity) {
        throw new CheckoutModuleError({
          message: 'Requested quantity exceeds available website inventory.',
          statusCode: 400,
          code: 'CHECKOUT_INSUFFICIENT_INVENTORY',
        });
      }

      const price = Number(product.sellingPrice);
      const subtotal = calculateItemSubtotal(price, quantity);
      const weight = normalizeWeight(item?.weight);

      snapshots.push({
        id: this.idGenerator(),
        productId,
        variantId,
        productName: product.name,
        variantName: `${variant.color} / ${variant.size}`,
        productImage: product.imageUrl || '',
        category: product.category || '',
        weight,
        sku: product.sku,
        qty: quantity,
        price,
        currency,
        subtotal,
      });
    }

    return snapshots;
  }

  async validateShipping({ shipping, courier, address, customer }) {
    const originDistrict = shipping?.originDistrict;
    const destinationDistrict = shipping?.destinationDistrict;
    const shippingCost = Number(shipping?.cost);
    const service = shipping?.service;
    const description = shipping?.description || '';
    const estimatedDelivery = shipping?.estimatedDelivery || '';
    const provinceId = address?.provinceId;
    const cityId = address?.cityId;
    const districtId = address?.districtId;
    const postalCode = address?.postalCode || '';
    const streetAddress = address?.streetAddress || '';
    const recipientName = String(address?.recipientName || customer?.customerName || '').trim();
    const phone = String(address?.phone || customer?.phone || '').trim();
    const shippingWeight = Number(shipping?.weight);

    if (!originDistrict || !destinationDistrict) {
      throw new CheckoutModuleError({
        message: 'Shipping originDistrict and destinationDistrict are required.',
        statusCode: 400,
        code: 'CHECKOUT_SHIPPING_ROUTE_REQUIRED',
      });
    }

    if (!courier) {
      throw new CheckoutModuleError({
        message: 'courier is required.',
        statusCode: 400,
        code: 'CHECKOUT_COURIER_REQUIRED',
      });
    }

    if (!Number.isFinite(shippingWeight) || shippingWeight <= 0) {
      throw new CheckoutModuleError({
        message: 'Shipping weight is invalid.',
        statusCode: 400,
        code: 'CHECKOUT_INVALID_WEIGHT',
      });
    }

    if (!provinceId || !cityId || !districtId || !streetAddress || !recipientName || !phone) {
      throw new CheckoutModuleError({
        message: 'Shipping address is incomplete.',
        statusCode: 400,
        code: 'CHECKOUT_ADDRESS_REQUIRED',
      });
    }

    if (!Number.isFinite(shippingCost) || shippingCost < 0) {
      throw new CheckoutModuleError({
        message: 'Shipping cost is invalid.',
        statusCode: 400,
        code: 'CHECKOUT_INVALID_SHIPPING_COST',
      });
    }

    const provinces = await this.shippingService.getProvinces();
    const province = provinces.find((entry) => entry.id === String(provinceId));
    if (!province) {
      throw new CheckoutModuleError({
        message: 'Shipping province was not found.',
        statusCode: 404,
        code: 'CHECKOUT_PROVINCE_NOT_FOUND',
      });
    }

    const cities = await this.shippingService.getCities(provinceId);
    const city = cities.find((entry) => entry.id === String(cityId));
    if (!city) {
      throw new CheckoutModuleError({
        message: 'Shipping city was not found.',
        statusCode: 404,
        code: 'CHECKOUT_CITY_NOT_FOUND',
      });
    }

    const districts = await this.shippingService.getDistricts(cityId);
    const district = districts.find((entry) => entry.id === String(districtId));
    if (!district) {
      throw new CheckoutModuleError({
        message: 'Shipping district was not found.',
        statusCode: 404,
        code: 'CHECKOUT_DISTRICT_NOT_FOUND',
      });
    }

    if (postalCode && district.postal_code && String(postalCode) !== String(district.postal_code)) {
      throw new CheckoutModuleError({
        message: 'Shipping postal code is invalid.',
        statusCode: 400,
        code: 'CHECKOUT_INVALID_POSTAL_CODE',
      });
    }

    console.log('[CheckoutService:validateShipping:request]', {
      originDistrict,
      destinationDistrict,
      courier,
      weight: shippingWeight,
      service,
      description,
      cost: shippingCost,
    });

    const executeShippingCost = this.getShippingCostExecutor();
    let availableRates;

    try {
      availableRates = await executeShippingCost({
        originDistrict,
        destinationDistrict,
        destinationPostalCode: postalCode,
        weight: shippingWeight,
        courier,
      });
    } catch (error) {
      console.warn('[CheckoutService:validateShipping:failure]', {
        originDistrict,
        destinationDistrict,
        courier,
        weight: shippingWeight,
        service,
        description,
        cost: shippingCost,
        validationError: error?.message || 'Unknown shipping validation error.',
        validationCode: error?.code || 'UNKNOWN_SHIPPING_VALIDATION_ERROR',
      });
      throw error;
    }

    if (!Array.isArray(availableRates) || availableRates.length === 0) {
      console.warn('[CheckoutService:validateShipping:failure]', {
        originDistrict,
        destinationDistrict,
        courier,
        weight: shippingWeight,
        service,
        description,
        cost: shippingCost,
        validationError: 'Shipping is unavailable for the selected destination.',
        validationCode: 'CHECKOUT_SHIPPING_UNAVAILABLE',
      });
      throw new CheckoutModuleError({
        message: 'Shipping is unavailable for the selected destination.',
        statusCode: 400,
        code: 'CHECKOUT_SHIPPING_UNAVAILABLE',
      });
    }

    const selectedRate = availableRates.find((rate) => {
      const sameCourier = String(rate.courier).toLowerCase() === String(courier).toLowerCase()
        || String(rate.courier).toUpperCase() === String(courier).toUpperCase();
      const sameService = service ? String(rate.service).toLowerCase() === String(service).toLowerCase() : true;
      return sameCourier && sameService && Number(rate.cost) === shippingCost;
    });

    if (!selectedRate) {
      console.warn('[CheckoutService:validateShipping:failure]', {
        originDistrict,
        destinationDistrict,
        courier,
        weight: shippingWeight,
        service,
        description,
        cost: shippingCost,
        validationError: 'Shipping rate is invalid.',
        validationCode: 'CHECKOUT_SHIPPING_RATE_INVALID',
        availableRates,
      });
      throw new CheckoutModuleError({
        message: 'Shipping rate is invalid.',
        statusCode: 400,
        code: 'CHECKOUT_SHIPPING_RATE_INVALID',
      });
    }

    return {
      recipientName,
      phone,
      originDistrict: String(originDistrict),
      destinationDistrict: String(destinationDistrict),
      shippingProvider: String(selectedRate.shippingProvider || shippingConfig.provider || 'rajaongkir'),
      shippingOriginId: String(selectedRate.originProviderId || originDistrict || ''),
      shippingDestinationId: String(selectedRate.destinationProviderId || postalCode || destinationDistrict || ''),
      courier: String(selectedRate.courier),
      service: String(selectedRate.service),
      description: String(description || selectedRate.description || ''),
      estimatedDelivery: String(estimatedDelivery || selectedRate.estimated_delivery || ''),
      shippingCost,
      provinceId: String(provinceId),
      provinceName: String(province.name),
      cityId: String(cityId),
      cityName: String(city.name),
      districtId: String(districtId),
      districtName: String(district.name),
      postalCode: String(postalCode || district.postal_code || ''),
      streetAddress: String(streetAddress),
    };
  }

  calculateTotals({ items, shippingCost, discount = 0, tax = 0 }) {
    const subtotal = calculateSubtotal(items);
    const normalizedDiscount = Number(discount) || 0;
    const normalizedTax = Number(tax) || 0;
    const grandTotal = calculateGrandTotal({
      subtotal,
      discount: normalizedDiscount,
      shippingCost,
      tax: normalizedTax,
    });

    return {
      subtotal,
      discount: normalizedDiscount,
      shipping: shippingCost,
      shippingCost,
      tax: normalizedTax,
      grandTotal,
      currency: DEFAULT_CURRENCY,
    };
  }

  buildExpirationDate() {
    const expiresAt = this.nowFactory();
    expiresAt.setHours(expiresAt.getHours() + DEFAULT_EXPIRATION_HOURS);
    return expiresAt;
  }

  buildCheckoutSessionResponse(session) {
    return {
      id: session.id,
      checkoutNumber: session.checkoutNumber,
      status: session.status,
      customer: {
        id: session.customerId,
        customerCode: session.customerCode,
        customerName: session.customerName,
        email: session.customerEmail,
        phone: session.customerPhone,
      },
      salesChannel: {
        id: session.salesChannelId,
        channelCode: session.salesChannelCode,
        channelName: session.salesChannelName,
      },
      currency: session.currency,
      items: session.items.map((item) => ({
        id: item.id,
        productId: item.productId,
        variantId: item.variantId,
        sku: item.sku,
        productName: item.productName,
        variantName: item.variantName,
        productImage: item.productImage,
        weight: item.weight,
        currency: item.currency,
        quantity: item.qty,
        qty: item.qty,
        price: item.price,
        subtotal: item.subtotal,
      })),
      promotion: session.promotionSnapshot || null,
      shipping: {
        recipientName: session.recipientName,
        phone: session.phone,
        originDistrict: session.originDistrict,
        destinationDistrict: session.destinationDistrict,
        courier: session.courier,
        service: session.courierService,
        description: session.shippingDescription,
        estimatedDelivery: session.estimatedDelivery,
        shippingCost: session.shippingCost,
        address: {
          provinceId: session.provinceId,
          province: session.provinceName,
          cityId: session.cityId,
          city: session.cityName,
          districtId: session.districtId,
          district: session.districtName,
          postalCode: session.postalCode,
          streetAddress: session.streetAddress,
        },
      },
      totals: {
        subtotal: session.subtotal,
        discount: session.discount,
        shipping: session.shippingCost,
        shippingCost: session.shippingCost,
        tax: session.tax,
        grandTotal: session.grandTotal,
        currency: session.currency,
      },
      processing: {
        processingKey: session.processingKey || null,
        processingStartedAt: session.processingStartedAt || null,
      },
      expiresAt: session.expiresAt,
      createdAt: session.createdAt,
      updatedAt: session.updatedAt,
    };
  }

  async syncCheckoutSessionExpiration(session) {
    if (!session) {
      return session;
    }

    const expiresAt = new Date(session.expiresAt);
    const isExpired = !Number.isNaN(expiresAt.getTime()) && expiresAt.getTime() <= this.nowFactory().getTime();
    const isExpirable = [CHECKOUT_STATUS.DRAFT, CHECKOUT_STATUS.PENDING].includes(session.status);

    if (!isExpired || !isExpirable) {
      return session;
    }

    const updatedSession = await this.prisma.$transaction(async (tx) => {
      const nextSession = await tx.checkoutSession.update({
        where: { id: session.id },
        data: { status: CHECKOUT_STATUS.EXPIRED },
        include: { items: true },
      });

      if (tx.paymentAttempt?.updateMany) {
        await tx.paymentAttempt.updateMany({
          where: {
            checkoutSessionId: session.id,
            status: { in: ['CREATED', 'PENDING'] },
          },
          data: { status: 'EXPIRED' },
        });
      }

      return nextSession;
    });

    return updatedSession;
  }

  async getLatestPaymentAttempt(checkoutSessionId) {
    if (!this.prisma.paymentAttempt?.findFirst) {
      return null;
    }

    return this.prisma.paymentAttempt.findFirst({
      where: { checkoutSessionId },
      orderBy: [{ createdAt: 'desc' }],
    });
  }

  buildCheckoutSessionStateResponse(session, paymentAttempt = null) {
    return {
      ...this.buildCheckoutSessionResponse(session),
      paymentAttemptId: paymentAttempt?.id || '',
      paymentAttempt: paymentAttempt ? {
        id: paymentAttempt.id,
        attemptNumber: paymentAttempt.attemptNumber,
        provider: paymentAttempt.provider,
        providerReference: paymentAttempt.providerReference,
        providerTransactionId: paymentAttempt.providerTransactionId,
        snapToken: paymentAttempt.snapToken,
        snapRedirectUrl: paymentAttempt.snapRedirectUrl,
        status: paymentAttempt.status,
        paymentMethod: paymentAttempt.paymentType || paymentAttempt.provider,
        grossAmount: paymentAttempt.grossAmount,
        currency: paymentAttempt.currency,
        expiresAt: paymentAttempt.expiresAt,
        createdAt: paymentAttempt.createdAt,
        updatedAt: paymentAttempt.updatedAt,
      } : null,
    };
  }

  async getCheckoutSessionStateById(checkoutSessionId) {
    if (!checkoutSessionId) {
      throw new CheckoutModuleError({
        message: 'checkoutSessionId is required.',
        statusCode: 400,
        code: 'CHECKOUT_SESSION_ID_REQUIRED',
      });
    }

    let session = await this.getCheckoutSessionRecord(checkoutSessionId);
    if (!session) {
      throw new CheckoutModuleError({
        message: 'Checkout session was not found.',
        statusCode: 404,
        code: 'CHECKOUT_SESSION_NOT_FOUND',
      });
    }

    session = await this.syncCheckoutSessionExpiration(session);
    const latestPaymentAttempt = await this.getLatestPaymentAttempt(checkoutSessionId);

    return this.buildCheckoutSessionStateResponse(session, latestPaymentAttempt);
  }

  async getCheckoutSessionRecord(checkoutSessionId) {
    return this.prisma.checkoutSession.findUnique({
      where: { id: checkoutSessionId },
      include: { items: true },
    });
  }

  async assertCheckoutSessionIsAvailable(session, {
    allowProcessingLock = true,
    allowedStatuses = [CHECKOUT_STATUS.DRAFT],
  } = {}) {
    if (!session) {
      throw new CheckoutModuleError({
        message: 'Checkout session was not found.',
        statusCode: 404,
        code: 'CHECKOUT_SESSION_NOT_FOUND',
      });
    }

    if (new Date(session.expiresAt).getTime() <= this.nowFactory().getTime()) {
      if (session.status !== CHECKOUT_STATUS.EXPIRED) {
        await this.prisma.checkoutSession.update({
          where: { id: session.id },
          data: { status: CHECKOUT_STATUS.EXPIRED },
        });
        session.status = CHECKOUT_STATUS.EXPIRED;
      }

      throw new CheckoutModuleError({
        message: 'Checkout session has expired.',
        statusCode: 410,
        code: 'CHECKOUT_SESSION_EXPIRED',
      });
    }

    if (!allowedStatuses.includes(session.status)) {
      throw new CheckoutModuleError({
        message: 'Checkout session cannot continue with the current status.',
        statusCode: 400,
        code: 'CHECKOUT_INVALID_STATUS',
      });
    }

    if (!allowProcessingLock && session.processingKey) {
      throw new CheckoutModuleError({
        message: 'Checkout session is already being processed.',
        statusCode: 409,
        code: 'CHECKOUT_ALREADY_PROCESSING',
      });
    }

    return session;
  }

  async getCheckoutSessionById(checkoutSessionId) {
    if (!checkoutSessionId) {
      throw new CheckoutModuleError({
        message: 'checkoutSessionId is required.',
        statusCode: 400,
        code: 'CHECKOUT_SESSION_ID_REQUIRED',
      });
    }

    const startedAt = Date.now();
    const session = await this.getCheckoutSessionRecord(checkoutSessionId);

    try {
      await this.assertCheckoutSessionIsAvailable(session);
      logCheckoutValidation({
        checkoutNumber: session.checkoutNumber,
        status: session.status,
        validationResult: 'PASSED',
        durationMs: Date.now() - startedAt,
      });
      return this.buildCheckoutSessionResponse(session);
    } catch (error) {
      logCheckoutValidation({
        checkoutNumber: session?.checkoutNumber || '',
        status: session?.status || '',
        validationResult: error.code === 'CHECKOUT_SESSION_EXPIRED' ? 'EXPIRED' : 'FAILED',
        durationMs: Date.now() - startedAt,
      });
      throw error;
    }
  }

  async getCheckoutSessionForPayment(checkoutSessionId) {
    if (!checkoutSessionId) {
      throw new CheckoutModuleError({
        message: 'checkoutSessionId is required.',
        statusCode: 400,
        code: 'CHECKOUT_SESSION_ID_REQUIRED',
      });
    }

    const startedAt = Date.now();
    const session = await this.getCheckoutSessionRecord(checkoutSessionId);

    try {
      await this.assertCheckoutSessionIsAvailable(session, {
        allowedStatuses: [CHECKOUT_STATUS.DRAFT, CHECKOUT_STATUS.PENDING],
      });
      logCheckoutValidation({
        checkoutNumber: session.checkoutNumber,
        status: session.status,
        validationResult: 'PASSED',
        durationMs: Date.now() - startedAt,
      });
      return this.buildCheckoutSessionResponse(session);
    } catch (error) {
      logCheckoutValidation({
        checkoutNumber: session?.checkoutNumber || '',
        status: session?.status || '',
        validationResult: error.code === 'CHECKOUT_SESSION_EXPIRED' ? 'EXPIRED' : 'FAILED',
        durationMs: Date.now() - startedAt,
      });
      throw error;
    }
  }

  async beginCheckoutProcessing(checkoutSessionId, processingKey) {
    if (!checkoutSessionId) {
      throw new CheckoutModuleError({
        message: 'checkoutSessionId is required.',
        statusCode: 400,
        code: 'CHECKOUT_SESSION_ID_REQUIRED',
      });
    }

    if (!processingKey) {
      throw new CheckoutModuleError({
        message: 'processingKey is required.',
        statusCode: 400,
        code: 'CHECKOUT_PROCESSING_KEY_REQUIRED',
      });
    }

    const startedAt = Date.now();
    const existingSession = await this.getCheckoutSessionRecord(checkoutSessionId);
    await this.assertCheckoutSessionIsAvailable(existingSession);

    if (existingSession.processingKey === processingKey) {
      logCheckoutValidation({
        checkoutNumber: existingSession.checkoutNumber,
        status: existingSession.status,
        validationResult: 'IDEMPOTENT_REUSE',
        durationMs: Date.now() - startedAt,
      });
      return {
        checkoutId: existingSession.id,
        checkoutNumber: existingSession.checkoutNumber,
        processingKey: existingSession.processingKey,
        processingStartedAt: existingSession.processingStartedAt,
        status: existingSession.status,
      };
    }

    if (existingSession.processingKey) {
      logCheckoutValidation({
        checkoutNumber: existingSession.checkoutNumber,
        status: existingSession.status,
        validationResult: 'LOCKED',
        durationMs: Date.now() - startedAt,
      });
      throw new CheckoutModuleError({
        message: 'Checkout session is already being processed.',
        statusCode: 409,
        code: 'CHECKOUT_ALREADY_PROCESSING',
      });
    }

    const processingStartedAt = this.nowFactory();
    const result = await this.prisma.checkoutSession.updateMany({
      where: {
        id: checkoutSessionId,
        status: CHECKOUT_STATUS.DRAFT,
        processingKey: null,
      },
      data: {
        processingKey,
        processingStartedAt,
      },
    });

    if (!result || result.count !== 1) {
      const refreshedSession = await this.getCheckoutSessionRecord(checkoutSessionId);

      if (refreshedSession?.processingKey === processingKey) {
        return {
          checkoutId: refreshedSession.id,
          checkoutNumber: refreshedSession.checkoutNumber,
          processingKey: refreshedSession.processingKey,
          processingStartedAt: refreshedSession.processingStartedAt,
          status: refreshedSession.status,
        };
      }

      if (refreshedSession?.processingKey) {
        throw new CheckoutModuleError({
          message: 'Checkout session is already being processed.',
          statusCode: 409,
          code: 'CHECKOUT_ALREADY_PROCESSING',
        });
      }

      await this.assertCheckoutSessionIsAvailable(refreshedSession);

      throw new CheckoutModuleError({
        message: 'Checkout processing could not be started.',
        statusCode: 409,
        code: 'CHECKOUT_PROCESSING_CONFLICT',
      });
    }

    const lockedSession = await this.getCheckoutSessionRecord(checkoutSessionId);
    logCheckoutValidation({
      checkoutNumber: lockedSession.checkoutNumber,
      status: lockedSession.status,
      validationResult: 'LOCKED',
      durationMs: Date.now() - startedAt,
    });

    return {
      checkoutId: lockedSession.id,
      checkoutNumber: lockedSession.checkoutNumber,
      processingKey: lockedSession.processingKey,
      processingStartedAt: lockedSession.processingStartedAt,
      status: lockedSession.status,
    };
  }

  async listCheckoutSessionsByCustomer({ customerId = '', email = '', page = 1, limit = 20 } = {}) {
    const normalizedPage = Math.max(1, Number.parseInt(String(page || '1'), 10) || 1);
    const normalizedLimit = Math.min(100, Math.max(1, Number.parseInt(String(limit || '20'), 10) || 20));
    const normalizedEmail = String(email || '').trim().toLowerCase();

    const where = {
      ...(customerId ? { customerId } : normalizedEmail ? { customerEmail: { equals: normalizedEmail, mode: 'insensitive' } } : {}),
      status: { in: [CHECKOUT_STATUS.DRAFT, CHECKOUT_STATUS.PENDING, CHECKOUT_STATUS.CANCELLED, CHECKOUT_STATUS.EXPIRED] },
    };

    const [totalItems, sessions] = await Promise.all([
      this.prisma.checkoutSession.count({ where }),
      this.prisma.checkoutSession.findMany({
        where,
        include: { items: true },
        orderBy: [{ createdAt: 'desc' }],
        skip: (normalizedPage - 1) * normalizedLimit,
        take: normalizedLimit,
      }),
    ]);

    const syncedSessions = await Promise.all(sessions.map((session) => this.syncCheckoutSessionExpiration(session)));
    const latestAttempts = await Promise.all(syncedSessions.map((session) => this.getLatestPaymentAttempt(session.id)));

    const data = syncedSessions.map((session, index) => {
      const paymentAttempt = latestAttempts[index];
      return {
        id: session.id,
        checkoutNumber: session.checkoutNumber,
        status: session.status,
        customerName: session.customerName,
        customerEmail: session.customerEmail,
        grandTotal: session.grandTotal,
        currency: session.currency,
        itemCount: session.items.length,
        paymentAttemptId: paymentAttempt?.id || '',
        paymentAttemptStatus: paymentAttempt?.status || '',
        paymentMethod: paymentAttempt?.paymentType || paymentAttempt?.provider || '',
        expiresAt: paymentAttempt?.expiresAt || session.expiresAt,
        createdAt: session.createdAt,
        updatedAt: session.updatedAt,
      };
    });

    const totalPages = Math.max(1, Math.ceil(totalItems / normalizedLimit));

    return {
      data,
      pagination: {
        page: normalizedPage,
        limit: normalizedLimit,
        totalItems,
        totalPages,
        hasNextPage: normalizedPage < totalPages,
        hasPreviousPage: normalizedPage > 1,
      },
    };
  }

  async listCheckoutSessionsForAdmin({ page = 1, limit = 20, search = '', status = 'all' } = {}) {
    const normalizedPage = Math.max(1, Number.parseInt(String(page || '1'), 10) || 1);
    const normalizedLimit = Math.min(100, Math.max(1, Number.parseInt(String(limit || '20'), 10) || 20));
    const normalizedSearch = String(search || '').trim();
    const normalizedStatus = String(status || 'all').trim().toUpperCase();

    const where = {
      ...(normalizedStatus && normalizedStatus !== 'ALL' ? { status: normalizedStatus } : {}),
      ...(normalizedSearch ? {
        OR: [
          { checkoutNumber: { contains: normalizedSearch, mode: 'insensitive' } },
          { customerName: { contains: normalizedSearch, mode: 'insensitive' } },
          { customerEmail: { contains: normalizedSearch, mode: 'insensitive' } },
        ],
      } : {}),
    };

    const [totalItems, sessions] = await Promise.all([
      this.prisma.checkoutSession.count({ where }),
      this.prisma.checkoutSession.findMany({
        where,
        include: { items: true },
        orderBy: [{ createdAt: 'desc' }],
        skip: (normalizedPage - 1) * normalizedLimit,
        take: normalizedLimit,
      }),
    ]);

    const syncedSessions = await Promise.all(sessions.map((session) => this.syncCheckoutSessionExpiration(session)));
    const [latestAttempts, customers, orders] = await Promise.all([
      Promise.all(syncedSessions.map((session) => this.getLatestPaymentAttempt(session.id))),
      this.prisma.customer.findMany({
        where: { id: { in: syncedSessions.map((session) => session.customerId) } },
        select: { id: true, isGuest: true },
      }),
      this.prisma.order.findMany({
        where: { checkoutSessionId: { in: syncedSessions.map((session) => session.id) } },
        select: { id: true, checkoutSessionId: true, publicOrderNumber: true, orderNumber: true },
      }),
    ]);

    const customerMap = new Map(customers.map((customer) => [customer.id, customer]));
    const orderMap = new Map(orders.map((order) => [order.checkoutSessionId, order]));

    const data = syncedSessions.map((session, index) => {
      const paymentAttempt = latestAttempts[index];
      const customer = customerMap.get(session.customerId);
      const order = orderMap.get(session.id);

      return {
        id: session.id,
        checkoutNumber: session.checkoutNumber,
        status: session.status,
        customerName: session.customerName,
        customerEmail: session.customerEmail,
        customerPhone: session.customerPhone,
        customerType: customer?.isGuest ? 'Guest' : 'Registered',
        grandTotal: session.grandTotal,
        currency: session.currency,
        itemCount: session.items.length,
        paymentAttempt: paymentAttempt ? {
          id: paymentAttempt.id,
          attemptNumber: paymentAttempt.attemptNumber,
          status: paymentAttempt.status,
          paymentMethod: paymentAttempt.paymentType || paymentAttempt.provider,
          expiresAt: paymentAttempt.expiresAt,
        } : null,
        expiredAt: paymentAttempt?.expiresAt || session.expiresAt,
        order: order ? {
          id: order.id,
          orderNumber: order.orderNumber,
          publicOrderNumber: order.publicOrderNumber,
        } : null,
        createdAt: session.createdAt,
        updatedAt: session.updatedAt,
      };
    });

    const totalPages = Math.max(1, Math.ceil(totalItems / normalizedLimit));

    return {
      data,
      pagination: {
        page: normalizedPage,
        limit: normalizedLimit,
        totalItems,
        totalPages,
        hasNextPage: normalizedPage < totalPages,
        hasPreviousPage: normalizedPage > 1,
      },
    };
  }

  async getCheckoutSessionAdminById(checkoutSessionId) {
    let session = await this.getCheckoutSessionRecord(checkoutSessionId);
    if (!session) {
      throw new CheckoutModuleError({
        message: 'Checkout session was not found.',
        statusCode: 404,
        code: 'CHECKOUT_SESSION_NOT_FOUND',
      });
    }

    session = await this.syncCheckoutSessionExpiration(session);

    const [paymentAttempt, customer, order] = await Promise.all([
      this.getLatestPaymentAttempt(checkoutSessionId),
      this.prisma.customer.findUnique({ where: { id: session.customerId }, select: { id: true, isGuest: true } }),
      this.prisma.order.findFirst({ where: { checkoutSessionId }, select: { id: true, orderNumber: true, publicOrderNumber: true, status: true } }),
    ]);

    return {
      ...this.buildCheckoutSessionStateResponse(session, paymentAttempt),
      customerClassification: customer?.isGuest ? 'Guest' : 'Registered',
      order: order || null,
    };
  }

  async createCheckoutSession(payload) {
    const startedAt = Date.now();
    let checkoutNumber = '';

    try {
      const currency = payload.currency || DEFAULT_CURRENCY;
      const customer = await this.ensureCheckoutCustomer({
        customerId: payload.authenticatedCustomerId || payload.customerId,
        customer: payload.customer,
      });
      const salesChannel = await this.validateSalesChannel(payload.salesChannelId);
      const items = await this.validateItems(payload.items, currency);
      const shipping = await this.validateShipping({
        shipping: payload.shipping,
        courier: payload.courier,
        address: payload.address,
        customer,
      });
      const subtotal = calculateSubtotal(items);
      const promotionSnapshot = await promotionService.buildPromotionSnapshotForCheckout({
        code: payload.promotionCode || '',
        customerId: customer.id,
        customerEmail: customer.email || '',
        subtotal,
        shippingCost: shipping.shippingCost,
        items,
        courier: shipping.courier,
      });
      const promotionPricing = promotionSnapshot?.pricing || null;
      const totals = this.calculateTotals({
        items,
        shippingCost: promotionPricing?.shippingCost ?? shipping.shippingCost,
        discount: promotionPricing?.discountAmount ?? payload.discount,
        tax: payload.tax,
      });
      const checkoutItems = items.map(({ category: _category, ...item }) => item);
      checkoutNumber = await this.generateCheckoutNumber();
      const expiresAt = this.buildExpirationDate();

      const session = await this.prisma.checkoutSession.create({
        data: {
          id: this.idGenerator(),
          checkoutNumber,
          status: CHECKOUT_STATUS.DRAFT,
          customerId: customer.id,
          customerCode: customer.customerCode || '',
          customerName: customer.customerName || '',
          customerEmail: customer.email || '',
          customerPhone: customer.phone || '',
          salesChannelId: salesChannel.id,
          salesChannelCode: salesChannel.channelCode || '',
          salesChannelName: salesChannel.channelName || '',
          currency,
          subtotal: totals.subtotal,
          shippingCost: totals.shippingCost,
          discount: totals.discount,
          tax: totals.tax,
          grandTotal: totals.grandTotal,
          promotionSnapshot: promotionSnapshot || null,
          recipientName: shipping.recipientName,
          phone: shipping.phone,
          originDistrict: shipping.originDistrict,
          destinationDistrict: shipping.destinationDistrict,
          courier: shipping.courier,
          courierService: shipping.service,
          shippingProvider: shipping.shippingProvider,
          shippingOriginId: shipping.shippingOriginId,
          shippingDestinationId: shipping.shippingDestinationId,
          shippingDescription: shipping.description,
          estimatedDelivery: shipping.estimatedDelivery,
          provinceId: shipping.provinceId,
          provinceName: shipping.provinceName,
          cityId: shipping.cityId,
          cityName: shipping.cityName,
          districtId: shipping.districtId,
          districtName: shipping.districtName,
          postalCode: shipping.postalCode,
          streetAddress: shipping.streetAddress,
          expiresAt,
          items: {
            create: checkoutItems,
          },
        },
        include: {
          items: true,
        },
      });

      logCheckoutValidation({
        checkoutNumber,
        status: session.status,
        validationResult: 'PASSED',
        durationMs: Date.now() - startedAt,
      });

      return this.buildCheckoutSessionResponse(session);
    } catch (error) {
      logCheckoutValidation({
        checkoutNumber,
        status: '',
        validationResult: 'FAILED',
        durationMs: Date.now() - startedAt,
      });
      console.error('[CheckoutService:createCheckoutSession:error]', {
        checkoutNumber,
        errorName: error?.name || '',
        errorCode: error?.code || '',
        errorMessage: error?.message || 'Unknown checkout session error.',
        errorStack: error?.stack || '',
        promotionCode: payload?.promotionCode || '',
        discount: payload?.discount,
        shippingCost: payload?.shipping?.cost,
        itemCount: Array.isArray(payload?.items) ? payload.items.length : 0,
      });
      throw error;
    }
  }
}

export const checkoutService = new CheckoutService();
