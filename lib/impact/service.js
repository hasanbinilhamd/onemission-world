...
async function loadPublicStories({ category = 'ALL' } = {}) {
  const normalizedCategory = String(category || 'ALL').trim().toUpperCase();
  const where = {
    status: { not: IMPACT_STATUS.DRAFT },
    ...(normalizedCategory !== 'ALL' && IMPACT_CATEGORIES.includes(normalizedCategory)
  ...
  return sortImpactStoriesForPublic(stories).map((story) =>
...
export const impactContentService = {
  ...
  async getPublicImpactList({ category = 'ALL', offset = 0, limit = 4 } = {}) {
    await ensureImpactDefaults();

    const normalizedOffset = Math.max(0, Number(offset) || 0);
    const normalizedLimit = Math.min(12, Math.max(1, Number(limit) || 4));

    const settings = await prisma.impactPageSetting.findUnique({ where: { id: 'impact' } });
    const sorted = await loadPublicStories({ category });
