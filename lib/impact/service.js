...
export const impactContentService = {
  ...
  async getPublicImpactList({ status = 'ALL', sort = 'latest', offset = 0, limit = 12 } = {}) {
    ...
    const pageAvailability = await pageAvailabilityService.getPageAvailability('impact');
  ...
  async updateImpactSettings({ settings = {}, user = null } = {}) {
    await prisma.impactPageSetting.upsert({
      ...
      create: { id: 'impact' },
      update: {
        eyebrow: normalizeText(settings.eyebrow, '', 240),
        title: normalizeText(settings.title, '', 240),
        description: normalizeText(settings.description, '', 500),
      },
    });
