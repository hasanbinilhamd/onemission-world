...
import { cashFlowService, inventoryValuationService, calculateProfitLossForPeriod } from '@/lib/finance-reporting';
...
function resolvePeriod({ range = 'last30', from = '', to = '', now = new Date() } = {}) {
  ...
  if (range === 'thisMonth') {
    ...
    return {
      ...
      from: formatDateOnly(fromDate),
      to: formatDateOnly(today),
      label: `This Month (${formatMonthLabel(now)})`,
    };
  }
...
export class DashboardService {
  ...
  async getExecutiveDashboardSummary({ range = 'last30', from = '', to = '' } = {}) {
    ...
    const [
      ...
      calculateProfitLossForPeriod({ periodStart: period.from, periodEnd: period.to, prismaClient: this.prisma }),
      calculateProfitLossForPeriod({ periodStart: formatDateOnly(currentMonthFrom), periodEnd: formatDateOnly(currentMonthTo), prismaClient: this.prisma }),
      calculateProfitLossForPeriod({ periodStart: formatDateOnly(previousMonthFrom), periodEnd: formatDateOnly(previousMonthTo), prismaClient: this.prisma }),
    ...
    const totalRevenue = periodProfitLoss.totalRevenue;
    const monthlyRevenue = currentMonthProfitLoss.totalRevenue;
    const previousMonthRevenue = previousMonthProfitLoss.totalRevenue;
    const revenueGrowth = calculateGrowth(monthlyRevenue, previousMonthRevenue);
    const totalCogs = periodProfitLoss.totalCogs;
    const totalExpenses = periodProfitLoss.totalExpenses;
    ...
    return {
      ...
      kpis: {
        ...
        netProfit: periodProfitLoss.netProfit,
        expenses: periodProfitLoss.totalOperatingExpenses,
        ...
        grossProfit: periodProfitLoss.grossProfit,
      },
    };
  }
  ...
  async getExecutiveDashboardDetails({ range = 'last30', from = '', to = '' } = {}) {
    ...
    const [
      ...
      this.prisma.returnRequest.groupBy({
        ...
        where: {
          updatedAt: { gte: period.fromDate, lte: period.toDate },
        },
        _count: { _all: true },
      }),
    ...
    const returnSummary = {
      refundRequested: groupedReturnSummary
        .filter((row) => row.refundStatus === 'REQUESTED')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
      refundApproved: groupedReturnSummary
        .filter((row) => row.refundStatus === 'APPROVED')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
      refundProcessing: groupedReturnSummary
        .filter((row) => row.refundStatus === 'PROCESSING')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
      refundCompleted: groupedReturnSummary
        .filter((row) => row.refundStatus === 'COMPLETED')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
      refundRejected: groupedReturnSummary
        .filter((row) => row.refundStatus === 'REJECTED')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
      refundFailed: groupedReturnSummary
        .filter((row) => row.refundStatus === 'FAILED')
        .reduce((sum, row) => sum + Number(row._count?._all || 0), 0),
