...
export class DashboardService {
  ...
  async getExecutiveDashboard({ range = 'last30', from = '', to = '' } = {}) {
    ...
    const totalRevenue = periodOrders.reduce((sum, order) => sum + Number(order.grandTotal || 0), 0);
    ...
    const totalExpenses = cashOutTransactions.reduce((sum, transaction) => sum + Number(transaction.amount || 0), 0);
    const netProfit = totalRevenue - totalCogs - totalExpenses;
    const cashPosition = Number(cashPositionReport?.closingCashPosition || 0);
    const lowStockRows = buildLowStockRows(inventoryRows);
    const lowStockCount = inventoryRows.filter((row) => Number(row.quantity || 0) <= Number(row.threshold || 0)).length;

    const completedProduction = completedProductionResultsThisMonth.length;
    const totalProductionCost = completedProductionResultsThisMonth.reduce(
      (sum, result) => sum + Number(result.totalProductionCost || 0),
      0,
    );
    const finishedGoodsProduced = completedProductionResultsThisMonth.reduce(
      (sum, result) => sum + Number(result.productionOrder?.actualQuantity || 0),
      0,
    );
