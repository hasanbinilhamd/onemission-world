  ...
  async getExecutiveDashboardDetails({ range = 'last30', from = '', to = '' } = {}) {
    ...
    const [
      ...
      this.prisma.contentPlanner.findMany({
        where: {
          publishDate: { startsWith: formatMonthKey(currentMonthFrom) },
        },
        select: {
          id: true,
          title: true,
    ...
    const contentPlannerSummary = {
      totalPlanned: contentPlannerThisMonth.length,
      published: contentPlannerThisMonth.filter((item) => item.status === 'Published').length,
      ready: contentPlannerThisMonth.filter((item) => item.status === 'Ready').length,
      editing: contentPlannerThisMonth.filter((item) => item.status === 'Editing').length,
      draft: contentPlannerThisMonth.filter((item) => ['Draft', 'Idea', 'Writing Script', 'Ready To Shoot'].includes(item.status)).length,
    };
