...
const LEGACY_TARGET_SCOPE_MAP = {
  SPECIFIC_PRODUCT: PROMOTION_TARGET_SCOPE.SELECTED_PRODUCTS,
  SPECIFIC_CATEGORY: PROMOTION_TARGET_SCOPE.SELECTED_CATEGORIES,
};
...
function normalizeNonNegativeNumber(value, fieldLabel) {
  const normalized = typeof value === 'string'
    ? value.trim().replace(/^rp\s*/i, '').replace(/\./g, '').replace(',', '.')
    : value;
...
const PROMOTION_BUSINESS_TIMEZONE_OFFSET_MINUTES = 7 * 60;

function hasExplicitTimezone(value) {
  return /(?:z|[+-]\d{2}:?\d{2})$/i.test(String(value || '').trim());
}

function parseBusinessLocalDate(value) {
  ...
  return new Date(utcTimestamp - (PROMOTION_BUSINESS_TIMEZONE_OFFSET_MINUTES * 60 * 1000));
}
...
function normalizeOptionalDate(value) {
  ...
  const parsed = hasExplicitTimezone(normalized)
    ? new Date(normalized)
    : (parseBusinessLocalDate(normalized) || new Date(normalized));
...
function normalizeTargetScope(value) {
  const normalized = String(value || '').trim().toUpperCase();
  const mapped = LEGACY_TARGET_SCOPE_MAP[normalized] || normalized;
  return Object.values(PROMOTION_TARGET_SCOPE).includes(mapped)
    ? mapped
...
function getTargetSubtotal(promotion, items, fallbackSubtotal) {
  ...
  if (normalizedPromotion.targetScope === PROMOTION_TARGET_SCOPE.SELECTED_CATEGORIES) {
...
function buildPricing({ subtotal, shippingCost, automaticDiscountAmount, voucherDiscountAmount, shippingDiscountAmount }) {
  ...
  const finalSubtotal = Math.max(0, Number(subtotal || 0) - discountAmount);
  const finalShippingCost = Math.max(0, Number(shippingCost || 0) - Number(shippingDiscountAmount || 0));
  const grandTotal = Math.max(0, finalSubtotal + finalShippingCost);
  const shippingDiscount = Number(shippingDiscountAmount || 0);
  return {
    ...
    totalSavings: discountAmount + shippingDiscount,
    grandTotal,
  };
}

function buildPromotionResult({ appliedPromotions, pricing }) {
  ...
  return {
    ...
    shippingDiscount: Number(pricing.shippingDiscountAmount || 0),
    finalSubtotal: Number(pricing.finalSubtotal ?? Math.max(0, Number(pricing.subtotal || 0) - Number(pricing.discountAmount || 0))),
    finalShipping: Number(pricing.finalShipping ?? pricing.shippingCost ?? 0),
    finalTotal: Number(pricing.finalTotal ?? pricing.grandTotal ?? 0),
  };
}
...
function buildPromotionSnapshot({ appliedPromotions, pricing }) {
  ...
  const promotionResult = buildPromotionResult({ appliedPromotions: applied, pricing });
  return {
    id: primary?.id || '',
    code: primary?.code || '',
    title: primary?.title || '',
...
function normalizePromotionPayload(input = {}) {
  ...
  const targetScope = promotionType === PROMOTION_TYPE.FREE_SHIPPING
    ? PROMOTION_TARGET_SCOPE.ENTIRE_STORE
...
async function calculatePromotionEnginePricing({ code = '', customerId = '', customerEmail = '', subtotal = 0, shippingCost = 0, items = [], courier = '' } = {}) {
  ...
  if (normalizedCode) {
    ...
    const voucherMinimumPurchase = Number(voucher.minimumPurchase || 0);
    if (normalizedSubtotal < voucherMinimumPurchase) {
      throw new PromotionError({
        message: `Minimum purchase is ${voucherMinimumPurchase.toLocaleString('id-ID')}.`,
        statusCode: 400,
    ...
    if (voucher.discountType === PROMOTION_DISCOUNT_TYPE.FREE_SHIPPING) {
      ...
      const voucherTargetAmount = getTargetSubtotal(voucher, loadedItems, normalizedSubtotal);
      if (voucherTargetAmount <= 0) {
        throw new PromotionError({
          message: 'Voucher is not applicable to the selected products.',
          statusCode: 400,
          code: 'PROMOTION_TARGET_NOT_MATCHED',
        });
      }
      voucherDiscountAmount = calculateDiscountForPromotion({ promotion: voucher, amount: voucherTargetAmount });
      appliedPromotions.push(buildAppliedPromotion({
        promotion: { ...voucher, usedCount },
  ...
  async validatePromotionPreview({ code = '', customerId = '', customerEmail = '', subtotal = 0, shippingCost = 0, items = [], courier = '' } = {}) {
    ...
    return getCachedPromotionValue(cacheKey, PROMOTION_VALIDATE_TTL_SECONDS, async () => {
      ...
      return {
        ...
        promotionResult: buildPromotionResult({
