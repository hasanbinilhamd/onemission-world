...
export class EarlyAccessService {
  ...
  async updateConfig({ enabled, chapter } = {}) {
    ...
    await this.prisma.$transaction([
      ...
      ...(chapterChanged ? [this.prisma.systemSetting.update({ where: { settingKey: SETTING_KEYS.revision }, data: { value: crypto.randomUUID() } })] : []),
    ]);
  ...
  async generatePassword() {
    ...
    await this.prisma.$transaction([
      this.prisma.systemSetting.update({ where: { settingKey: SETTING_KEYS.passwordHash }, data: { value: passwordHash } }),
      this.prisma.systemSetting.update({ where: { settingKey: SETTING_KEYS.revision }, data: { value: crypto.randomUUID() } }),
    ]);
