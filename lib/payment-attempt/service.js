...
export class PaymentAttemptService {
  constructor({
    ...
    this.onRefundCompleted = onRefundCompleted;
  }
  ...
  async requestRefund({ paymentAttemptId, amount, reason = '', refundKey = '' }) {
    const attempt = await this.getPaymentAttemptById(paymentAttemptId);
    ...
    if (attempt.status === 'REFUNDED') {
      return {
        providerReference: attempt.providerReference,
        refundReference: '',
        refundProviderId: attempt.providerTransactionId || '',
        amount: Number(amount || attempt.grossAmount || 0),
        createdAt: this.nowFactory().toISOString(),
        httpStatus: 200,
        providerPayload: attempt.providerPayload || null,
        statusCode: '200',
        statusMessage: 'Refund already completed.',
        transactionId: attempt.providerTransactionId || '',
        requestBody: null,
        responseBody: attempt.providerPayload || null,
        responseAt: this.nowFactory().toISOString(),
    ...
    if (attempt.status !== 'PAID') {
      throw new PaymentAttemptError({
        message: 'Refund can only be requested for paid payment attempts.',
        statusCode: 409,
        code: 'PAYMENT_ATTEMPT_REFUND_NOT_ALLOWED',
      });
    }

    const providerReference = String(attempt.providerReference || '').trim();
    if (!providerReference) {
      throw new PaymentAttemptError({
        message: 'Payment provider reference is missing for refund processing.',
        statusCode: 409,
        code: 'PAYMENT_ATTEMPT_REFUND_REFERENCE_MISSING',
      });
    }

    const refundAmount = Number(amount || attempt.grossAmount || 0);
    ...
    return this.paymentProvider.requestRefund({
      providerReference,
      transactionId: attempt.providerTransactionId || '',
      refundKey: String(refundKey || '').trim() || `refund-${attempt.attemptNumber}-${Date.now()}`,
      amount: refundAmount,
      reason: String(reason || '').trim() || 'Customer refund request approved by HQ.',
    });
  }
    ...
    try {
      ...
      if (nextStatus === 'REFUNDED' || nextStatus === 'CANCELLED') {
        ...
        if (nextStatus === 'REFUNDED') {
          const refundedOrder = await this.onRefundCompleted(workingAttempt, normalizedNotification);
          orderId = refundedOrder?.id || relatedOrder?.id || '';
          orderNumber = refundedOrder?.orderNumber || relatedOrder?.orderNumber || '';
        }
        ...
        if (relatedOrder || orderNumber) {
          await notificationService.dispatch({
            ...
            payload: {
              orderNumber: orderNumber || relatedOrder?.orderNumber || '',
      ...
      if (SUCCESSFUL_PAYMENT_STATUSES.includes(nextStatus)) {
        ...
        if (existingOrder) {
          ...
          const recoveredOrder = await this.onPaymentConfirmed(workingAttempt);
          const recoveryAction = recoveredOrder?.__meta?.action || 'FOUND';
          const recoveryInventoryReservationResult = recoveredOrder?.__meta?.inventoryReservationResult
            || (recoveredOrder?.__meta?.inventoryCommitted ? 'COMMITTED' : 'SKIPPED');
          ...
          logWebhookPhase({
            ...
            orderNumber,
            providerReference,
            inventoryReservationResult: recoveryInventoryReservationResult,
            durationMs: Date.now() - startedAt,
            validationResult: recoveryAction === 'FOUND' ? 'REUSED_AND_RECOVERED' : 'RECOVERED',
          });
        } else {
