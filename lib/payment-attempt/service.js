...
function isTransitionAllowed(currentStatus, nextStatus) {
  const allowedStatuses =
    PAYMENT_ATTEMPT_STATUS_TRANSITIONS[currentStatus] || [];
...
export class PaymentAttemptService {
  ...
  validatePaymentAttemptForSnap(attempt) {
    ...
    if (!SNAP_GENERATABLE_STATUSES.includes(attempt.status)) {
      throw new PaymentAttemptError({
        message:
          "Payment attempt cannot generate Snap token with the current status.",
        statusCode: 400,
  ...
  async createPaymentAttempt({ checkoutSessionId }) {
    ...
    try {
      ...
      const checkoutSession =
        await this.checkoutService.getCheckoutSessionForPayment(
          checkoutSessionId,
        );
  ...
  async generateSnapToken({ paymentAttemptId }) {
    ...
    try {
      ...
      const providerReference =
        attempt.providerReference || attempt.attemptNumber;
  ...
  async handleMidtransNotification(payload) {
    ...
    try {
      ...
      const normalizedNotification =
        await this.paymentProvider.normalizeNotification(payload);
      providerReference = normalizedNotification.providerReference;
      nextStatus = normalizedNotification.internalStatus;

      const attempt =
        await this.getPaymentAttemptByProviderReference(providerReference);
      attemptNumber = attempt.attemptNumber;
      previousStatus = attempt.status;
      ...
      const updatedAttempt = await this.prisma.paymentAttempt.update({
        ...
        data: {
          providerReference:
            normalizedNotification.providerReference ||
            attempt.providerReference,
          providerTransactionId:
            normalizedNotification.providerTransactionId ||
            attempt.providerTransactionId,
          fraudStatus:
            normalizedNotification.fraudStatus || attempt.fraudStatus || "",
          paymentType:
            normalizedNotification.paymentType || attempt.paymentType || "",
          transactionTime:
            normalizedNotification.transactionTime ||
            attempt.transactionTime ||
            null,
          settlementTime:
            normalizedNotification.settlementTime ||
            attempt.settlementTime ||
            null,
          providerPayload:
            normalizedNotification.providerPayload ||
            attempt.providerPayload ||
            null,
          status: nextStatus,
        },
      });
