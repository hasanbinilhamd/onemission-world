...
import { paymentAttemptConfig, getMidtransRefundBaseUrl, getMidtransSnapBaseUrl } from '../config';
...
function buildCallbacks() {
  return {
    finish: paymentAttemptConfig.callbackBaseUrl,
    unfinish: paymentAttemptConfig.callbackBaseUrl,
    error: paymentAttemptConfig.callbackBaseUrl,
  };
}
...
function buildRefundErrorMetadata(providerMessage = '') {
  ...
  if (normalizedMessage.includes('insufficient') && normalizedMessage.includes('balance')) {
    return {
      code: 'PAYMENT_ATTEMPT_REFUND_INSUFFICIENT_BALANCE',
      message: 'Refund failed because the Midtrans merchant balance is insufficient.',
      retryAvailable: true,
    };
  }

  if (normalizedMessage.includes('transaction') && normalizedMessage.includes('not found')) {
    return {
      code: 'PAYMENT_ATTEMPT_REFUND_TRANSACTION_NOT_FOUND',
      message: 'Refund failed because the payment transaction could not be found in Midtrans.',
      retryAvailable: false,
    };
  }

  if (normalizedMessage.includes('amount') && normalizedMessage.includes('invalid')) {
    return {
      code: 'PAYMENT_ATTEMPT_REFUND_AMOUNT_INVALID',
      message: 'Refund failed because the refund amount is invalid.',
      retryAvailable: false,
    };
  }

  if (normalizedMessage.includes('refund_key') || normalizedMessage.includes('duplicate refund key')) {
    return {
      code: 'PAYMENT_ATTEMPT_REFUND_DUPLICATE_KEY',
      message: 'Refund failed because the refund key is duplicated.',
      retryAvailable: true,
    };
  }
  ...
  if (normalizedMessage.includes('reject')) {
    return {
      code: 'PAYMENT_ATTEMPT_REFUND_REJECTED',
      message: 'Refund failed because Midtrans rejected the refund request.',
      retryAvailable: false,
    };
  }

  return {
    code: 'PAYMENT_ATTEMPT_REFUND_GATEWAY_ERROR',
    message: 'Refund failed because Midtrans could not process the refund request.',
    retryAvailable: true,
  };
}

function normalizeMidtransRefundError(responseStatus, responseBody = '', requestBody = null) {
  const parsedBody = parseMidtransResponseBody(responseBody);
  const providerMessage = extractProviderMessage(parsedBody, responseBody);
  const metadata = buildRefundErrorMetadata(providerMessage);

  const error = buildProviderError({
    message: metadata.message,
    statusCode: responseStatus >= 500 ? 503 : 502,
    code: metadata.code,
    httpStatus: responseStatus,
    providerMessage,
    responseBody,
  ...
  error.statusCodeValue = String(parsedBody?.status_code || responseStatus || '');
  error.statusMessageValue = String(parsedBody?.status_message || providerMessage || '');
  error.requestBody = requestBody;
  error.responseBodyJson = parsedBody || null;
  error.responseAt = new Date().toISOString();
  error.gatewayName = 'MIDTRANS';
  error.retryAvailable = metadata.retryAvailable;
  return error;
}
...
function normalizeMidtransRefundNetworkError(error, requestBody = null) {
  if (error instanceof PaymentAttemptError) {
    error.requestBody = error.requestBody ?? requestBody;
    error.gatewayName = error.gatewayName || 'MIDTRANS';
    error.retryAvailable = error.retryAvailable ?? true;
    return error;
  }

  const normalized = error?.name === 'AbortError'
    ? buildProviderError({
        message: 'Refund failed because Midtrans did not respond in time.',
        statusCode: 504,
        code: 'PAYMENT_ATTEMPT_REFUND_TIMEOUT',
        providerMessage: 'Request timed out while waiting for Midtrans refund response.',
        cause: error,
      })
    : buildProviderError({
        message: 'Refund failed because Midtrans is currently unavailable.',
        statusCode: 503,
        code: 'PAYMENT_ATTEMPT_REFUND_PROVIDER_UNAVAILABLE',
        providerMessage: error?.message || 'Network connection to Midtrans failed.',
        cause: error,
      });

  normalized.statusCodeValue = '';
  normalized.statusMessageValue = normalized.providerMessage;
  normalized.requestBody = requestBody;
  normalized.responseBodyJson = null;
  normalized.responseAt = new Date().toISOString();
  normalized.gatewayName = 'MIDTRANS';
  normalized.retryAvailable = true;
  return normalized;
}
...
function normalizeNotificationPayload(payload) {
  ...
  return {
    ...
    refundKey: String(payload?.refund_key || payload?.refundKey || '').trim(),
    refundAmount: parseMidtransAmount(payload?.refund_amount || payload?.gross_amount),
    refundTransactionId: String(payload?.refund_chargeback_id || payload?.refund_id || payload?.refundTransactionId || '').trim(),
    refundDate: parseMidtransDate(payload?.refund_at || payload?.refund_time || payload?.settlement_time || payload?.transaction_time),
...
export class MidtransProvider extends PaymentProvider {
  ...
  async requestRefund(payload) {
    ...
    try {
      response = await fetch(getMidtransRefundBaseUrl(payload.providerReference), {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      throw normalizeMidtransRefundNetworkError(error, requestBody);
    } finally {
      clearTimeout(timeoutId);
    }
    ...
    const parsedBody = parseMidtransResponseBody(responseBody);
    if (!response.ok) {
      console.error('[PaymentAttempt:MidtransProvider:Refund]', {
        orderNumber: payload.providerReference,
        refundKey: payload.refundKey,
        status: response.status,
        responseBody,
      });
      throw normalizeMidtransRefundError(response.status, responseBody, requestBody);
    }

    const data = parsedBody || {};
    const providerStatusCode = String(data.status_code || response.status || '');
    if (!['200', '201'].includes(providerStatusCode)) {
      throw normalizeMidtransRefundError(response.status, responseBody, requestBody);
    }
    ...
    return {
      ...
      refundReference: String(data.refund_key || payload.refundKey || ''),
      midtransRefundId: String(data.refund_chargeback_id || data.refund_id || ''),
      refundProviderId: String(data.refund_chargeback_id || data.refund_id || data.transaction_id || ''),
      transactionId: String(data.transaction_id || payload.transactionId || ''),
      amount: Number(data.refund_amount || payload.amount || 0),
      createdAt: new Date().toISOString(),
      httpStatus: response.status,
