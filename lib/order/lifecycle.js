...
const LEGACY_FULFILLMENT_STATUS_MAP = {
  PENDING_PAYMENT: FULFILLMENT_STATUS.WAITING_PAYMENT,
...
export const FULFILLMENT_TO_ORDER_STATUS = {
  [FULFILLMENT_STATUS.WAITING_PAYMENT]: ORDER_STATUS.PENDING_PAYMENT,
  ...
  [FULFILLMENT_STATUS.CANCELLED]: ORDER_STATUS.CANCELLED,
};
...
export const FULFILLMENT_STATUS_TRANSITIONS = {
  ...
  // separate Picking and Packing phases.
  [FULFILLMENT_STATUS.WAITING_PAYMENT]: [FULFILLMENT_STATUS.WAITING_PAYMENT, FULFILLMENT_STATUS.CANCELLED],
  [FULFILLMENT_STATUS.PENDING]: [FULFILLMENT_STATUS.PENDING, FULFILLMENT_STATUS.PACKING, FULFILLMENT_STATUS.CANCELLED],
  [FULFILLMENT_STATUS.PICKING]: [FULFILLMENT_STATUS.PICKING, FULFILLMENT_STATUS.PACKING, FULFILLMENT_STATUS.CANCELLED],
  [FULFILLMENT_STATUS.PACKING]: [FULFILLMENT_STATUS.PACKING, FULFILLMENT_STATUS.READY_TO_SHIP, FULFILLMENT_STATUS.CANCELLED],
  [FULFILLMENT_STATUS.READY_TO_SHIP]: [FULFILLMENT_STATUS.READY_TO_SHIP, FULFILLMENT_STATUS.SHIPPED, FULFILLMENT_STATUS.CANCELLED],
  ...
  [FULFILLMENT_STATUS.CANCELLED]: [FULFILLMENT_STATUS.CANCELLED],
};

export const FULFILLMENT_TIMELINE_EVENTS = {
  [FULFILLMENT_STATUS.WAITING_PAYMENT]: 'WAITING_PAYMENT',
  ...
  [FULFILLMENT_STATUS.CANCELLED]: 'FULFILLMENT_CANCELLED',
};
...
export const ORDER_TO_FULFILLMENT_STATUS = {
  [ORDER_STATUS.PENDING_PAYMENT]: FULFILLMENT_STATUS.WAITING_PAYMENT,
  [ORDER_STATUS.PAID]: FULFILLMENT_STATUS.PENDING,
  [ORDER_STATUS.CANCELLED]: FULFILLMENT_STATUS.CANCELLED,
};
...
export function getSynchronizedFulfillmentStatus({ orderStatus = '', fulfillmentStatus = '' } = {}) {
  const normalizedOrderStatus = normalizeOrderStatusValue(orderStatus);
  const normalizedFulfillmentStatus = normalizeFulfillmentStatusValue(fulfillmentStatus);

  if (ORDER_TO_FULFILLMENT_STATUS[normalizedOrderStatus]) {
    return ORDER_TO_FULFILLMENT_STATUS[normalizedOrderStatus];
  }

  return normalizedFulfillmentStatus || FULFILLMENT_STATUS.PENDING;
}
...
export function getFulfillmentStatusQueryValues(status) {
  ...
  switch (normalized) {
    ...
    case FULFILLMENT_STATUS.WAITING_PAYMENT:
      return [FULFILLMENT_STATUS.WAITING_PAYMENT];
