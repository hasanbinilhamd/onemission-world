...
export class OrderService {
  ...
  buildOrderResponse(order) {
    return {
      ...
      status: order.status,
      fulfillmentStatus: order.fulfillmentStatus || FULFILLMENT_STATUS.PENDING,
      shipment: {
  ...
  async listOrders({ page = 1, limit = 10, search = '', sortBy = 'createdAt', sortOrder = 'desc' } = {}) {
    ...
    const where = normalizedSearch
      ? {
          OR: [
            ...
            { paymentReference: { contains: normalizedSearch, mode: 'insensitive' } },
  ...
  async updateFulfillmentStatus({
    ...
    try {
      ...
      const normalizedStatus = String(fulfillmentStatus).trim().toUpperCase();
      ...
      const nextShippingDate = parsedShippingDate && !Number.isNaN(parsedShippingDate.getTime())
        ? parsedShippingDate
        : (nextStatus === FULFILLMENT_STATUS.SHIPPED && previousStatus !== FULFILLMENT_STATUS.SHIPPED
          ? this.nowFactory()
      ...
      if (nextStatus === FULFILLMENT_STATUS.SHIPPED || previousStatus === FULFILLMENT_STATUS.SHIPPED) {
        ...
        shipmentUpdates.shippingDate = nextShippingDate;
      }
