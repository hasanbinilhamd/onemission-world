    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.journalEntry.create({
        data: {
          ...
          description: `Manual refund paid for order ${returnRequest.order.publicOrderNumber || returnRequest.order.orderNumber}`,
          ...
          lines: {
            create: [
              { id: this.idGenerator(), chartOfAccountId: refundExpense.id, description: 'Manual refund paid', debitAmount: amount, creditAmount: 0 },
              { id: this.idGenerator(), chartOfAccountId: cashAccount.id, description: 'Manual refund paid', debitAmount: 0, creditAmount: amount },
            ],
          },
        },
      });
