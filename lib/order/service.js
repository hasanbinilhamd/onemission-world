...
const FULFILLMENT_STATUS_LABELS = {
  ...
  [FULFILLMENT_STATUS.PROCESSING]: 'PROCESSING',
  [FULFILLMENT_STATUS.PACKED]: 'PACKED',
  [FULFILLMENT_STATUS.SHIPPED]: 'SHIPPED',
  [FULFILLMENT_STATUS.COMPLETED]: 'COMPLETED',
};

const FULFILLMENT_STATUS_TRANSITIONS = {
  [FULFILLMENT_STATUS.PENDING]: [FULFILLMENT_STATUS.PENDING, FULFILLMENT_STATUS.PROCESSING],
  [FULFILLMENT_STATUS.PROCESSING]: [FULFILLMENT_STATUS.PROCESSING, FULFILLMENT_STATUS.PACKED],
  [FULFILLMENT_STATUS.PACKED]: [FULFILLMENT_STATUS.PACKED, FULFILLMENT_STATUS.SHIPPED],
  [FULFILLMENT_STATUS.SHIPPED]: [FULFILLMENT_STATUS.SHIPPED, FULFILLMENT_STATUS.COMPLETED],
  [FULFILLMENT_STATUS.COMPLETED]: [FULFILLMENT_STATUS.COMPLETED],
};
...
function normalizeFulfillmentStatusValue(value) {
  const normalized = String(value || '').trim().toUpperCase();
  if (normalized === 'READY_FOR_FULFILLMENT') {
...
function toFulfillmentStatusLabel(status) {
  return FULFILLMENT_STATUS_LABELS[status] || status || 'READY_FOR_FULFILLMENT';
}

function isFulfillmentTransitionAllowed(currentStatus, nextStatus) {
  const allowed = FULFILLMENT_STATUS_TRANSITIONS[currentStatus] || [];
  return allowed.includes(nextStatus);
}

function mapTimelineEventName(fulfillmentStatus) {
  const eventMap = {
    [FULFILLMENT_STATUS.PENDING]: 'Ready For Fulfillment',
    [FULFILLMENT_STATUS.PROCESSING]: 'Processing',
    [FULFILLMENT_STATUS.PACKED]: 'Packed',
    [FULFILLMENT_STATUS.SHIPPED]: 'Shipped',
    [FULFILLMENT_STATUS.COMPLETED]: 'Completed',
  ...
  return eventMap[fulfillmentStatus] || fulfillmentStatus;
}
...
export class OrderService {
  ...
  buildOrderResponse(order) {
    const normalizedFulfillmentStatus = order.fulfillmentStatus || FULFILLMENT_STATUS.PENDING;
    ...
    return {
      ...
      fulfillmentStatusLabel: toFulfillmentStatusLabel(normalizedFulfillmentStatus),
      shipment: {
  ...
  async listOrders({
    ...
    const [totalItems, orders, readyForFulfillment, processing, packed, shipped, completed] = await Promise.all([
      ...
      this.prisma.order.count({ where: { ...where, fulfillmentStatus: FULFILLMENT_STATUS.PENDING } }),
      this.prisma.order.count({ where: { ...where, fulfillmentStatus: FULFILLMENT_STATUS.PROCESSING } }),
      this.prisma.order.count({ where: { ...where, fulfillmentStatus: FULFILLMENT_STATUS.PACKED } }),
      this.prisma.order.count({ where: { ...where, fulfillmentStatus: FULFILLMENT_STATUS.SHIPPED } }),
      this.prisma.order.count({ where: { ...where, fulfillmentStatus: FULFILLMENT_STATUS.COMPLETED } }),
    ]);
  ...
  async createFromCheckoutSession({ paymentAttemptId }) {
    ...
    try {
      ...
      const existingOrder = await this.getExistingOrderForPaymentAttempt(paymentAttempt);
        ...
        if (isOrderUniqueConstraintError(error)) {
          ...
          if (concurrentOrder) {
            logOrderCreation({
              ...
              checkoutNumber: concurrentOrder.checkoutSessionId,
              paymentAttemptNumber,
              durationMs: Date.now() - startedAt,
              validationResult: 'REUSED',
            });
            return attachInternalOrderMetadata(this.buildOrderResponse(concurrentOrder), {
              action: 'FOUND',
              inventoryCommitted: false,
            });
          }
      ...
      return attachInternalOrderMetadata(this.buildOrderResponse(order), {
        action: 'CREATED',
  ...
  async updateFulfillmentStatus({
    ...
    try {
      ...
      const order = await this.getOrderRecord(orderId);
      orderNumber = order.orderNumber;
      previousStatus = order.fulfillmentStatus || FULFILLMENT_STATUS.PENDING;
      ...
      if (nextStatus === FULFILLMENT_STATUS.SHIPPED || previousStatus === FULFILLMENT_STATUS.SHIPPED) {
      ...
      const timelineEntries = previousStatus === nextStatus
        ? []
        : [buildTimelineEntry({
            id: this.idGenerator(),
            orderId: order.id,
            eventName: mapTimelineEventName(nextStatus),
      ...
      logFulfillmentUpdate({
        orderNumber,
        previousStatus: toFulfillmentStatusLabel(previousStatus),
        newStatus: toFulfillmentStatusLabel(nextStatus),
        updatedBy: normalizedUpdatedBy,
