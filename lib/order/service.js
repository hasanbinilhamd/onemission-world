...
function buildReturnTimelineNotes({ reason = '', description = '', rejectReason = '', refundStatus = '', refundAmount = 0, refundReference = '', refundProviderId = '' } = {}) {
  const lines = [];
...
export class OrderService {
  ...
  async approveReturnRequest({ returnRequestId, updatedBy = '' }) {
    ...
    if (
      returnRequest.status !== RETURN_REQUEST_STATUS.REQUESTED
      && !(returnRequest.status === RETURN_REQUEST_STATUS.APPROVED && currentRefundStatus === RETURN_REFUND_STATUS.APPROVED)
    ) {
      throw new OrderError({
    ...
    const refundAmount = Number(returnRequest.refundAmount || approvedOrder?.grandTotal || 0);
    const paymentAttemptId = approvedOrder?.paymentAttemptId || approvedOrder?.paymentAttempt?.id || '';
    if (!paymentAttemptId) {
      throw new OrderError({
    ...
    let refundResult;
    try {
      refundResult = await this.paymentAttemptService.requestRefund({
        paymentAttemptId,
        amount: refundAmount,
        reason: returnRequest.reason || 'Customer refund approved by HQ.',
      });
    } catch (error) {
      return this.buildOrderResponse(approvedOrder);
    }

    const processingAt = this.nowFactory();
    const processingOrder = await this.prisma.$transaction(async (tx) => {
      await tx.returnRequest.update({
        where: { id: returnRequest.id },
        data: {
          refundStatus: RETURN_REFUND_STATUS.PROCESSING,
          refundProcessingAt: processingAt,
  ...
  async rejectReturnRequest({ returnRequestId, rejectReason = '', updatedBy = '' }) {
    ...
    if (
      ...
      throw new OrderError({
        message: 'Only requested refund requests can be rejected.',
        statusCode: 409,
  ...
  async getReturnSummary() {
    const [refundRequested, refundProcessing, refundCompleted, refundRejected, cancelledOrders] = await Promise.all([
