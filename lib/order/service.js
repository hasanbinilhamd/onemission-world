...
export class OrderService {
  ...
  async markManualRefundPaid({ returnRequestId, refundAmount = 0, refundMethod = '', refundDate = null, refundReference = '', note = '', updatedBy = '' }) {
    ...
    const txResult = await this.prisma.$transaction(async (tx) => {
      const fresh = await tx.returnRequest.findUnique({ where: { id: returnRequest.id } });
      if (fresh.financeTransactionId || fresh.refundStatus === RETURN_REFUND_STATUS.PAID) throw new OrderError({ message: 'Refund has already been paid.', statusCode: 409, code: 'RETURN_REFUND_ALREADY_PAID' });
  ...
  async markReplacementSent({ returnRequestId, replacementItems = [], note = '', updatedBy = '' }) {
    ...
    const now = this.nowFactory();
    await this.prisma.$transaction(async (tx) => {
      await tx.returnRequest.update({ where: { id: returnRequest.id }, data: { status: RETURN_REQUEST_STATUS.COMPLETED, replacementStatus: 'SENT', replacementSentAt: now, replacementNote: String(note || '').trim(), replacementItems: Array.isArray(replacementItems) ? replacementItems : [], completedAt: now } });
