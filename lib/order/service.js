...
export class OrderService {
  ...
  async requestReturnByCustomer({
    ...
    const normalizedResolution = String(resolution || 'REFUND').trim().toUpperCase() === 'REPLACEMENT' ? 'REPLACEMENT' : 'REFUND';
    ...
    if (normalizedResolution === 'REPLACEMENT') {
      ...
      for (const replacement of requestedReplacementItems) {
        ...
        if (!originalOrderItem) throw new OrderError({ message: 'Replacement original item is invalid.', statusCode: 400, code: 'RETURN_REPLACEMENT_ORIGINAL_ITEM_INVALID' });
        ...
        const replacementProduct = await this.prisma.product.findUnique({ where: { id: replacementProductId } });
        const replacementVariant = await this.prisma.inventory.findUnique({ where: { id: replacementVariantId } });
        if (!replacementProduct || !replacementVariant || replacementVariant.productId !== replacementProductId) {
        ...
        const originalUnitPrice = Number(originalOrderItem.price || 0);
        const replacementUnitPrice = Number(replacementProduct.sellingPrice || 0);
        const originalAmount = originalUnitPrice * replacementQuantity;
        const replacementAmount = replacementUnitPrice * replacementQuantity;
        replacementSnapshotItems.push({
          originalOrderItemId,
          originalProductId: originalOrderItem.productId,
          originalVariantId: originalOrderItem.variantId,
          originalProductName: originalOrderItem.productName,
          originalVariantName: originalOrderItem.variantName,
          ...
          note: String(replacement.note || replacement.replacementNote || '').trim(),
          status: 'REQUESTED',
        });
      }
    }
  ...
  async resolveRefundExpenseAccount(tx) {
    const account = await tx.chartOfAccount.findFirst({
      where: { isActive: true, allowTransaction: true, accountType: 'Expense', OR: [{ accountName: { contains: 'Refund', mode: 'insensitive' } }, { accountName: { contains: 'Return', mode: 'insensitive' } }] },
      orderBy: { accountCode: 'asc' },
    }) || await tx.chartOfAccount.findFirst({ where: { isActive: true, allowTransaction: true, accountType: 'Expense' }, orderBy: { accountCode: 'asc' } });
    if (!account) throw new OrderError({ message: 'Refund expense account could not be resolved.', statusCode: 400, code: 'REFUND_FINANCE_ACCOUNT_NOT_FOUND' });
    return account;
  }

  async markManualRefundPaid({ returnRequestId, refundAmount = 0, refundMethod = '', refundDate = null, refundReference = '', note = '', updatedBy = '' }) {
    const returnRequest = await this.prisma.returnRequest.findUnique({ where: { id: returnRequestId }, include: { order: true } });
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const cashAccount = await this.financePostingService.resolveCashOrBankAccount(tx);
      const financialAccount = await tx.financialAccount.findFirst({ where: { isActive: true, linkedCoaId: cashAccount.id }, orderBy: { name: 'asc' } })
        || await tx.financialAccount.findFirst({ where: { isActive: true }, orderBy: { name: 'asc' } });
      ...
      const refundExpense = await this.resolveRefundExpenseAccount(tx);
      const cashTxn = await tx.cashTransaction.create({ data: { id: this.idGenerator(), transactionDate: paidAt.toISOString().slice(0,10), transactionType: 'OUT', financialAccountId: financialAccount.id, chartOfAccountId: refundExpense.id, amount, referenceNumber: String(refundReference || '').trim(), description: `Manual refund for return ${returnRequest.id} / order ${returnRequest.order.publicOrderNumber || returnRequest.order.orderNumber}`, vendor: returnRequest.order.customerName || '', paymentMethod: String(refundMethod || 'BANK_TRANSFER').trim(), notes: String(note || '').trim(), createdBy: actor } });
      const journalNumber = await this.financePostingService.generateJournalNumber(tx, paidAt);
      await tx.journalEntry.create({ data: { id: this.idGenerator(), journalNumber, journalDate: paidAt.toISOString().slice(0,10), description: `Manual refund paid for order ${returnRequest.order.publicOrderNumber || returnRequest.order.orderNumber}`, referenceNumber: String(refundReference || cashTxn.id), journalSource: 'Return Refund', sourceId: cashTxn.id, journalType: 'System', status: 'Posted', totalDebit: amount, totalCredit: amount, createdBy: actor, updatedBy: actor, lines: { create: [ { id: this.idGenerator(), chartOfAccountId: refundExpense.id, description: 'Manual refund paid', debitAmount: amount, creditAmount: 0 }, { id: this.idGenerator(), chartOfAccountId: cashAccount.id, description: 'Manual refund paid', debitAmount: 0, creditAmount: amount } ] } } });
      await tx.returnRequest.update({ where: { id: returnRequest.id }, data: { status: RETURN_REQUEST_STATUS.COMPLETED, refundStatus: RETURN_REFUND_STATUS.PAID, refundCompletedAt: paidAt, completedAt: paidAt, manualRefundMethod: String(refundMethod || 'BANK_TRANSFER').trim(), manualRefundDate: paidAt, manualRefundReference: String(refundReference || '').trim(), manualRefundNote: String(note || '').trim(), manualRefundPaidBy: actor, financeTransactionId: cashTxn.id, refundAmount: amount } });
      await tx.orderTimeline.create({ data: buildTimelineEntry({ id: this.idGenerator(), orderId: returnRequest.orderId, eventName: 'REFUND_PAID', updatedBy: actor, notes: `Refund paid manually. Amount: ${amount.toLocaleString('id-ID')}`, createdAt: paidAt }) });
      return cashTxn;
    });
    const order = await this.prisma.order.findUnique({ where: { id: returnRequest.orderId }, include: ORDER_DETAIL_INCLUDE });
    return this.buildOrderResponse(order);
  ...
  async updateFulfillmentStatus({
    ...
    try {
      ...
      const hasShipmentInput = Boolean(normalizedShipmentCourier || normalizedShipmentService || normalizedTrackingNumber || normalizedShippingDate);
