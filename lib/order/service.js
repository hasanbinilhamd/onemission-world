...
import { getSystemSettingsMap } from '@/lib/hq-security';
...
function getOrderReturnLifecycleStatus(order) {
  ...
  if (returnRequest.refundStatus === 'PROCESSING') {
    return ORDER_STATUS.REFUND_PROCESSING;
  }
  if (returnRequest.status === 'APPROVED') {
    return ORDER_STATUS.RETURN_APPROVED;
  }
  if (returnRequest.status === 'REJECTED') {
    return ORDER_STATUS.RETURN_REJECTED;
  }
...
function getOrderDisplayStatus(order) {
  const returnLifecycleStatus = getOrderReturnLifecycleStatus(order);
  if (returnLifecycleStatus) {
    return returnLifecycleStatus;
  }
...
function canCustomerCancelOrder(order) {
  ...
  const fulfillmentStatus = normalizeFulfillmentStatusValue(order?.fulfillmentStatus) || FULFILLMENT_STATUS.PENDING;

  if (getOrderReturnLifecycleStatus(order)) {
    return false;
  }
...
function buildReturnTimelineNotes({ reason = '', description = '', rejectReason = '', refundStatus = '' } = {}) {
  const lines = [];
...
export class OrderService {
  ...
  async generateOrderNumber() {
    ...
    const configuredPrefix = String(systemSettings.order_prefix?.value || 'ORD').trim().toUpperCase() || 'ORD';
    ...
    const updatedOrder = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.orderTimeline.create({
        data: buildTimelineEntry({
          ...
          notes: buildReturnTimelineNotes({ description: normalizedReason }),
      ...
      await tx.orderTimeline.createMany({
        data: [
          buildTimelineEntry({
            ...
            updatedBy: order.customerName || order.customerEmail || ORDER_TIMELINE_SYSTEM_USER,
            notes: buildReturnTimelineNotes({ reason: normalizedReason, description: normalizedDescription }),
            createdAt: requestedAt,
          }),
          buildTimelineEntry({
            id: this.idGenerator(),
            orderId: order.id,
            eventName: 'RETURN_PENDING_REVIEW',
            updatedBy: ORDER_TIMELINE_SYSTEM_USER,
            notes: 'Waiting Seller Review',
    ...
    const updatedOrder = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.orderTimeline.create({
        data: buildTimelineEntry({
          ...
          notes: buildReturnTimelineNotes({ refundStatus: 'NONE' }),
    ...
    const updatedOrder = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.orderTimeline.create({
        data: buildTimelineEntry({
          ...
          updatedBy: normalizedUpdatedBy,
          notes: buildReturnTimelineNotes({ rejectReason: normalizedRejectReason }),
  ...
  async updateReturnRefundStatus({ returnRequestId, refundStatus = '', updatedBy = '' }) {
    const returnRequest = await this.prisma.returnRequest.findUnique({ where: { id: returnRequestId } });
    if (!returnRequest) {
      throw new OrderError({
        message: 'Return request was not found.',
    ...
    const normalizedRefundStatus = String(refundStatus || '').trim().toUpperCase();
    if (!['NONE', 'PROCESSING', 'COMPLETED'].includes(normalizedRefundStatus)) {
      throw new OrderError({
        message: 'Refund status is invalid.',
        statusCode: 400,
        code: 'RETURN_REQUEST_REFUND_STATUS_INVALID',
      });
    }

    if (normalizedRefundStatus !== 'NONE' && returnRequest.status !== 'APPROVED') {
      throw new OrderError({
        message: 'Refund status can only be updated after the return has been approved.',
        statusCode: 409,
        code: 'RETURN_REQUEST_REFUND_STATUS_NOT_ALLOWED',
      });
    }
    ...
    const updatedOrder = await this.prisma.$transaction(async (tx) => {
      await tx.returnRequest.update({
        ...
        data: {
          ...
          completedAt: normalizedRefundStatus === 'COMPLETED' ? updatedAt : returnRequest.completedAt,
        },
      });

      if (normalizedRefundStatus === 'PROCESSING' || normalizedRefundStatus === 'COMPLETED') {
        await tx.orderTimeline.create({
          data: buildTimelineEntry({
            id: this.idGenerator(),
            orderId: returnRequest.orderId,
            eventName: normalizedRefundStatus === 'PROCESSING' ? 'REFUND_PROCESSING' : 'REFUND_COMPLETED',
            updatedBy: normalizedUpdatedBy,
            notes: buildReturnTimelineNotes({ refundStatus: normalizedRefundStatus }),
            createdAt: updatedAt,
          }),
        });
      }
    ...
    if (normalizedRefundStatus === 'COMPLETED') {
      await this.notificationService.dispatch({
        type: NOTIFICATION_EVENT_TYPE.ORDER_REFUNDED,
        payload: {
          orderNumber: updatedOrder.orderNumber,
          publicOrderNumber: updatedOrder.publicOrderNumber,
        },
  ...
  async getReturnSummary() {
    const [pendingReturn, approvedReturn, refundProcessing, refundCompleted, cancelledOrders] = await Promise.all([
      this.prisma.returnRequest.count({ where: { status: 'REQUESTED' } }),
      this.prisma.returnRequest.count({ where: { status: 'APPROVED' } }),
      this.prisma.returnRequest.count({ where: { refundStatus: 'PROCESSING' } }),
      this.prisma.returnRequest.count({ where: { refundStatus: 'COMPLETED' } }),
