...
import {
  ...
  CONTENT_SCRIPT_LEGACY_CATEGORY_MAP,
...
function resolveMappedContentScriptCategory(category) {
  const normalizedCategory = normalizeString(category);
  return CONTENT_SCRIPT_LEGACY_CATEGORY_MAP[normalizedCategory] || null;
}

function normalizeContentScriptCategory(category) {
  return resolveMappedContentScriptCategory(category) || 'Story';
}
...
function buildContentScriptSummary(items = [], referenceDate = new Date()) {
  ...
  return {
    ...
    storyCount: items.filter((item) => normalizeContentScriptCategory(item.category) === 'Story').length,
    educationCount: items.filter((item) => normalizeContentScriptCategory(item.category) === 'Education').length,
    proofenCount: items.filter((item) => normalizeContentScriptCategory(item.category) === 'Proofen').length,
    productCount: items.filter((item) => normalizeContentScriptCategory(item.category) === 'Product').length,
    communityCount: items.filter((item) => normalizeContentScriptCategory(item.category) === 'Community').length,
  };
}
...
function validateContentScriptPayload(payload = {}, { requirePdf = true } = {}) {
  ...
  if (!category || !resolveMappedContentScriptCategory(category)) {
