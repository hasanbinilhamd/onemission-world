...
export const missionContentService = {
  ...
  async getPublicMissionContent({ customerId = null, anonymousVoterId = null } = {}) {
    // CMS is the single source of truth: only an OPEN mission is public
    // voting content. An empty database (or no OPEN mission) returns an
    // empty structure — never auto-created draft content.
    const mission = await prisma.mission.findFirst({
      where: { status: MISSION_STATUS.OPEN },
      orderBy: { updatedAt: 'desc' },
    });

    const pageAvailability = await pageAvailabilityService.getPageAvailability('mission');
    ...
    // Whether the current voter identity (authenticated OR anonymous) has
    // already voted in this mission. Exposed as a boolean only — the voter
    // identity itself is never returned.
    let hasVoted = false;
    const identity = resolveVoterIdentity({ customerId, anonymousVoterId });
    if (identity.ok) {
      const existingVote = await prisma.missionVote.findFirst({
        where: identity.identity.customerId
          ? { missionId: mission.id, customerId: identity.identity.customerId }
          : { missionId: mission.id, anonymousVoterId: identity.identity.anonymousVoterId },
        select: { id: true },
      });
  ...
  async recordVote({ missionOptionId, customerId = null, anonymousVoterId = null }) {
    const identity = resolveVoterIdentity({ customerId, anonymousVoterId });
    if (!identity.ok) {
      throw new MissionContentError({
        message: 'A valid voter identity is required to vote.',
        statusCode: 400,
        code: identity.code,
      });
    }
    ...
    try {
      await prisma.missionVote.create({
        data: {
          ...
          customerId: identity.identity.customerId,
          anonymousVoterId: identity.identity.anonymousVoterId,
        },
      });
    } catch (error) {
      if (isUniqueConstraintError(error)) {
        throw new MissionContentError({
          message: 'You have already voted for this mission.',
          statusCode: 409,
          code: 'MISSION_ALREADY_VOTED',
        });
      }
      throw error;
    }
