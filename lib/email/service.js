...
import { buildOrderStatusUpdateEmailHtml, buildOrderStatusUpdateEmailText } from './templates/order-status-update';
import { buildPasswordResetEmailHtml, buildPasswordResetEmailText } from './templates/password-reset';
import { buildRegisterOtpEmailHtml, buildRegisterOtpEmailText } from './templates/register-otp';
...
export class EmailService {
  ...
  async sendRegisterOtpEmail({ to, customerName, otp }) {
    return this.sendMail({
      to,
      subject: 'Verify your ONEMISSION Account',
      html: buildRegisterOtpEmailHtml({ customerName, otp }),
      text: buildRegisterOtpEmailText({ customerName, otp }),
    });
  }
  ...
  async sendOrderStatusUpdateEmail({ to, order, statusType }) {
    return this.sendMail({
      to,
      subject: statusType === 'SHIPPED' ? 'ONEMISSION Order Shipped' : 'ONEMISSION Order Delivered',
      html: buildOrderStatusUpdateEmailHtml({ order, statusType }),
      text: buildOrderStatusUpdateEmailText({ order, statusType }),
    });
  }
}
