...
const DEFAULT_NOTIFICATION_SETTINGS = [
  ...
  { category: 'internal', key: 'internal_payment_received', label: 'Payment Received', isEnabled: true },
  { category: 'internal', key: 'internal_order_cancelled', label: 'Order Cancelled', isEnabled: true },
  { category: 'internal', key: 'internal_refund_requested', label: 'Refund Requested', isEnabled: true },
  { category: 'internal', key: 'internal_order_refunded', label: 'Order Refunded', isEnabled: true },
  { category: 'internal', key: 'internal_order_delivered', label: 'Order Delivered', isEnabled: true },
  { category: 'internal', key: 'internal_out_of_stock', label: 'Out of Stock', isEnabled: true },
  { category: 'internal', key: 'internal_manual_stock_adjustment', label: 'Manual Stock Adjustment', isEnabled: true },
  { category: 'internal', key: 'internal_cash_out_approval', label: 'Cash Out Approval (Future)', isEnabled: false },
  { category: 'internal', key: 'internal_production_started', label: 'Production Started', isEnabled: true },
  { category: 'internal', key: 'internal_production_finished', label: 'Production Finished', isEnabled: true },
  { category: 'internal', key: 'internal_cash_in_created', label: 'Cash In Created', isEnabled: true },
  { category: 'internal', key: 'internal_cash_out_created', label: 'Cash Out Created', isEnabled: true },
  { category: 'internal', key: 'internal_user_created', label: 'User Created', isEnabled: true },
  { category: 'internal', key: 'internal_role_updated', label: 'Role Updated', isEnabled: true },
];
...
const ROLE_PERMISSIONS_CACHE_TTL_MS = 300_000;
...
async function getRolePermissionMap(prismaClient = prisma, { forceRefresh = false } = {}) {
  if (prismaClient !== prisma) {
    const roles = await prismaClient.role.findMany({ include: { permissions: true } });
  ...
  if (!forceRefresh && rolePermissionsCache && Date.now() < rolePermissionsCacheExpiresAt) {
    return rolePermissionsCache;
  }

  const roles = await prismaClient.role.findMany({ include: { permissions: true } });
  rolePermissionsCache = roles.reduce((map, role) => {
    map[role.name] = role.permissions || [];
    return map;
  }, {});
  rolePermissionsCacheExpiresAt = Date.now() + ROLE_PERMISSIONS_CACHE_TTL_MS;
  return rolePermissionsCache;
}
...
export async function authenticateHqRequest(request, { prismaClient = prisma, optional = false, touchSession = true } = {}) {
  ...
  const rolePermissionMap = await getRolePermissionMap(prismaClient);
  const permissions = rolePermissionMap[session.user.role] || [];
