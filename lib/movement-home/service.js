...
function normalizeHomePageInput(input = {}) {
  return {
    headline: normalizeText(input.headline, DEFAULT_HOME_PAGE.headline, 240),
    description: normalizeText(input.description, DEFAULT_HOME_PAGE.description, 500),
    ctaLabel: normalizeText(input.ctaLabel, DEFAULT_HOME_PAGE.ctaLabel, 80),
    ctaDestination: normalizeDestination(input.ctaDestination),
    socialProofNumber: normalizeText(input.socialProofNumber, DEFAULT_HOME_PAGE.socialProofNumber, 40),
    socialProofText: normalizeText(input.socialProofText, DEFAULT_HOME_PAGE.socialProofText, 160),
...
export const movementHomeContentService = {
  async getPublicHomeContent() {
    await ensureMovementHomeDefaults();
    ...
    const payload = {
      home: toPublicHomePage(homePage || DEFAULT_HOME_PAGE),
      cards: (cards.length > 0 ? cards : DEFAULT_HOME_CARDS).map(toPublicCard),
    };
  ...
  async updateHomePage({ home, user = null } = {}) {
    ...
    await prisma.homePage.upsert({
      where: { id: 'home' },
      create: { ...DEFAULT_HOME_PAGE, ...normalized },
      update: { ...normalized },
    });
